...
class SimilaritySearcher(ProcessorBase):
    ...
    def search_similar(self, query_point: KnowledgePoint, candidate_points: list[KnowledgePoint] | None = None) -> SimilarityResult:
        ...
        for candidate in candidate_points:
            ...
            candidate_vector = self.content_vectors.get(candidate.id, self._generate_content_vector(candidate.content))
    ...
    def _determine_match_type(self, matching_features: list[str]) -> str:
        ...
        any(f.startswith("same_type") for f in matching_features)
    ...
    def _calculate_cluster_similarity(self, cluster_ids: list[str], semantic_matches: list[SemanticMatch]) -> float:
        ...
        """
        cluster_matches = [match for match in semantic_matches if match.source_id in cluster_ids and match.target_id in cluster_ids]
    ...
    def _get_dominant_match_type(self, cluster_ids: list[str], semantic_matches: list[SemanticMatch]) -> str:
