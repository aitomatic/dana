...
from frameworks.knows.core.base import KnowledgePoint, ProcessorBase
...
class KnowledgeCategorizer(ProcessorBase):
    ...
    def __init__(self, categories: list[KnowledgeCategory] | None = None, similarity_threshold: float = 0.6):
    ...
    def process(self, knowledge_points: list[KnowledgePoint]) -> dict[str, Any]:
        ...
        try:
            ...
            for kp in knowledge_points:
                ...
                categorized_points.append({"knowledge_point": kp, "categories": category_assignments})
                ...
                for assignment in category_assignments:
                    rel = CategoryRelationship(
                        ...
                        metadata={"matching_keywords": assignment.get("matching_keywords", []), "assignment_method": "keyword_similarity"},
                    )
            ...
            DANA_LOGGER.info(
                f"Categorized {len(knowledge_points)} knowledge points into {len(set(r.category_id for r in relationships))} categories"
            )
            return result
    ...
    def _categorize_knowledge_point(self, knowledge_point: KnowledgePoint) -> list[dict[str, Any]]:
        ...
        for category in self.categories:
            ...
            if similarity_score >= self.similarity_threshold:
                ...
                matching_keywords = [kw for kw in category.keywords if kw in combined_text]
    ...
    def _map_knowledge_point_relationships(self, knowledge_points: list[KnowledgePoint]) -> list[dict[str, Any]]:
        ...
        for i, kp1 in enumerate(knowledge_points):
            for _j, kp2 in enumerate(knowledge_points[i + 1 :], start=i + 1):
                    ...
                    relationships.append(
                        {
                            ...
                            "strength": similarity,
                            "metadata": {"method": "content_similarity", "threshold": 0.7},
                        }
                    )
                    ...
                    relationships.append(
                        {
                            ...
                            "strength": 0.8,
                            "metadata": {"method": "contextual_analysis", "shared_context": self._find_shared_context(kp1, kp2)},
                        }
                    )
    ...
    def _build_category_hierarchy(self) -> dict[str, Any]:
        ...
        """
        hierarchy = {"root_categories": [], "category_tree": {}}
    ...
    def _generate_categorization_summary(
        self, categorized_points: list[dict[str, Any]], relationships: list[CategoryRelationship]
    ) -> dict[str, Any]:
