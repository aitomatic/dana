...
from frameworks.knows.core.base import Document, ParsedDocument, ProcessorBase
...
class DocumentParser(ProcessorBase):
    ...
    def process(self, document: Document) -> ParsedDocument:
        ...
        try:
            ...
            parsed_doc = ParsedDocument(
                ...
                metadata={"parser_version": "1.0", "parsed_elements": len(structured_data), "format": document.format},
            )
    ...
    def _parse_text_document(self, document: Document) -> dict[str, Any]:
        ...
        structured_data = {"type": "text_document", "sections": [], "headers": [], "lists": [], "metadata": {}}

        # Extract headers (markdown style)
        headers = re.findall(r"^(#{1,6})\s+(.+)$", content, re.MULTILINE)
        for level_marks, title in headers:
            structured_data["headers"].append({"level": len(level_marks), "title": title.strip(), "type": "header"})
        ...
        if structured_data["headers"]:
            ...
            for i, section in enumerate(sections[1:]):  # Skip first empty section
                if section.strip():
                    structured_data["sections"].append({"index": i, "content": section.strip(), "type": "section"})
        else:
            # No headers found, treat as single section
            structured_data["sections"].append({"index": 0, "content": content.strip(), "type": "section"})
        ...
        if list_items:
            structured_data["lists"].append({"type": "unordered", "items": list_items})

        if numbered_items:
            structured_data["lists"].append({"type": "ordered", "items": numbered_items})
    ...
    def _parse_csv_document(self, document: Document) -> dict[str, Any]:
        ...
        structured_data = {
            ...
            "headers": headers,
            "rows": rows,
            "metadata": {"column_count": len(headers), "row_count": len(rows), "total_lines": len(lines)},
        }
    ...
    def _analyze_json_schema(self, data: Any, path: str = "") -> dict[str, Any]:
        ...
        if isinstance(data, dict):
            schema = {"type": "object", "properties": {}, "path": path}
            ...
            schema = {"type": "array", "length": len(data), "path": path}
            if data:
            ...
            return {"type": type(data).__name__, "value": str(data) if len(str(data)) < 100 else str(data)[:100] + "...", "path": path}
