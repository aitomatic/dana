                    ...
                    try:
                        ...
                        return ExpertResponse(
                            ...
                            analysis={
                                ...
                                "limitations": [limitation.strip() for limitation in limitations.split('-') if limitation.strip()]
                            },
                            recommendations=[
                                {
                                    "suggestion": r.strip(),
        ...
        if 'confidence' in context and context['confidence'] < 0.7:
            return {
                "user_interaction": {
                    ...
                    "question": (
                        "I'm not fully confident about this approach. "
                        "Would you like me to explain my concerns?"
                    ),
...
class APIModelUsingAgent(ModelUsingAgent):
    ...
    async def _send_update(self, data: Dict) -> Dict:
        ...
        try:
            async with self.http_client.post(self.callback_url, json=data) as response:
            ...
            self.logger.error("Error in API call: %s", str(e))
            return {"status": "error", "error": str(e)}

    async def _get_user_response(self, interaction_request: Dict) -> Optional[str]:
        """Get response from user through API."""
        try:
            ...
            self.logger.error("Error getting user response: %s", str(e))
            return None
...
class WebSocketModelUsingAgent(ModelUsingAgent):
    ...
    async def start_session(self, initial_problem: Optional[str] = None):
        """Start a WebSocket-based interaction session."""
        await self.connect()
        ...
        try:
            if not initial_problem:
                await self._send_message({
                    ...
                    "message": "Please describe the problem you need help with:"
                })
                
                response = await self._receive_message()
                if response.get("type") == "problem_statement":
                    initial_problem = response.get("content")
                else:
                    raise ValueError("Invalid response type for problem statement")
    ...
    async def _solve_problem(self):
        """Main problem-solving loop for WebSocket-based interaction."""
        try:
            cycle_count = 0
            while True:
                cycle_count += 1
                ...
                if solution_found:
                    ...
                    response = await self._receive_message()
                    if response.get("type") == "solution_accepted":
                        self.logger.info("Solution accepted")
                        break
                    elif response.get("type") == "solution_feedback":
            ...
            self.logger.error("Error in problem-solving loop: %s", str(e))
            await self._send_message({
                "type": "error",
                "error": str(e)
            })
    ...
    async def _handle_stuck(self) -> bool:
        """Handle stuck state through WebSocket."""
        try:
            help_needed = self.state.working_memory.get('help_needed', 'proceeding')
            await self._send_message({
                "type": "stuck",
                "message": help_needed,
                "options": [
            ...
            self.logger.error("Error handling stuck state: %s", str(e))
            return False

    def _is_stuck(self) -> bool:
        """Determine if the agent is stuck and needs help."""
        return super()._is_stuck()  # Use parent class implementation
