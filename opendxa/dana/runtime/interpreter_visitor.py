"""DANA Runtime Interpreter using the visitor pattern.

This module provides a visitor-based implementation of the DANA interpreter.
It replicates the functionality of the original interpreter but uses the
visitor pattern for better extensibility.
"""
...
from opendxa.dana.runtime.interpreter import COLORS, RESET, format_error_location

class InterpreterVisitor(ASTVisitor):
    """Visitor implementation for interpreting DANA programs.
    
    This visitor traverses the AST and executes each node using the runtime
    context. It maintains compatibility with the original interpreter's behavior.
    """

    def __init__(self, context: RuntimeContext):
        """Initialize the interpreter visitor with a runtime context.
    ...
    def execute_program(self, parse_result: ParseResult) -> None:
        """Execute a DANA program using the visitor pattern.
...
def create_interpreter_visitor(context: RuntimeContext) -> InterpreterVisitor:
    """Create a new interpreter visitor with the given context."""
    return InterpreterVisitor(context)