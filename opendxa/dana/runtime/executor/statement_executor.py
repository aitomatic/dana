...
from opendxa.dana.runtime.hooks import has_hooks, HookType
...
class StatementExecutor(BaseExecutor):
    ...
    def execute(self, node: Any, context: Optional[Dict[str, Any]] = None) -> None:
        ...
        try:
            ...
            if isinstance(node, Assignment):
                self.execute_assignment(node, context)
            elif isinstance(node, LogStatement):
                self.execute_log_statement(node, context)
            elif isinstance(node, LogLevelSetStatement):
                self.execute_log_level_set_statement(node, context)
                ...
                self.execute_reason_statement(node, context)
            elif isinstance(node, FunctionCall):
                self.execute_function_call(node, context)
            else:
    ...
    def execute_assignment(self, node: Assignment, context: Optional[Dict[str, Any]] = None) -> None:
    ...
    def execute_log_statement(self, node: LogStatement, context: Optional[Dict[str, Any]] = None) -> None:
        ...
        Raises:
            RuntimeError: If the log statement fails
        """
        try:
            ...
            error, passthrough = handle_execution_error(e, node, "executing log statement")
            if passthrough:
                raise error
            else:
                raise RuntimeError(f"Failed to execute log statement: {e}")
    
    def execute_log_level_set_statement(self, node: LogLevelSetStatement, context: Optional[Dict[str, Any]] = None) -> None:
        """Execute a log level set statement.
        
        Args:
            node: The log level set statement to execute
        ...
        try:
            ...
            # Also update the log level in the other components
            self.expression_evaluator._log_level = node.level
            self.llm_integration._log_level = node.level
        except Exception as e:
            error, passthrough = handle_execution_error(e, node, "setting log level")
            if passthrough:
                raise error
            else:
                raise RuntimeError(f"Failed to set log level: {e}")
    ...
    def _execute_reason_statement_sync(self, node: ReasonStatement, context: Optional[Dict[str, Any]] = None) -> Any:
        ...
        self._log(f"Reasoning prompt: {prompt_str[:100]}{'...' if len(prompt_str) > 100 else ''}", LogLevel.DEBUG)
        ...
        result = self.llm_integration.execute_direct_synchronous_reasoning(
            prompt_str, context_vars, node.options
        )
        ...
        try:
            ...
            if "." not in node.name and has_function(node.name):
                ...
                if result is not None:
                    self._log(f"Function {node.name} returned: {result}", LogLevel.DEBUG)
        except Exception as e:
