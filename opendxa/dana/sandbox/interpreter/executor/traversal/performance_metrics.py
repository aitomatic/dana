...
class TraversalPerformanceMetrics(Loggable):
    ...
    def get_hot_node_types(self, top_n: int = 10) -> list[tuple[str, int]]:
        ...
        return sorted(
            self.execution_count_by_node_type.items(),
            key=lambda x: x[1],
            reverse=True
        )[:top_n]
    ...
    def get_slow_node_types(self, top_n: int = 10) -> list[tuple[str, float]]:
        ...
        return sorted(
            self.execution_time_by_node_type.items(),
            key=lambda x: x[1],
            reverse=True
        )[:top_n]
    ...
    def get_cache_efficiency_by_node_type(self) -> dict[str, dict[str, Any]]:
        ...
        for node_type in self.execution_count_by_node_type:
            ...
            efficiency[node_type] = {
                ...
                "avg_time": (
                    self.execution_time_by_node_type[node_type] / 
                    self.execution_count_by_node_type[node_type]
                    if self.execution_count_by_node_type[node_type] > 0 else 0.0
                ),
            }
    ...
    def get_overall_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "average_execution_time": (
                self.total_execution_time / total_executions 
            ...
            "performance_efficiency": (
                total_executions / self.total_execution_time 
                if self.total_execution_time > 0 else 0.0
            ),
        }
    ...
    def log_performance_report(self) -> None:
        ...
        self.info(
            f"Total: {stats['total_executions']} executions in {stats['elapsed_time']:.3f}s "
            f"({stats['executions_per_second']:.1f} exec/s)"
        )
        self.info(
            f"Cache: {stats['cache_hit_rate']:.1%} hit rate from {stats['cache_requests']} requests"
        )
        self.info(
            f"Performance: {stats['performance_efficiency']:.1f} exec/ms average"
        )
