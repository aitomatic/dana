"""
Semiconductor Domain Plugin for POET
...
class SemiconductorPlugin:
    ...
    def validate_output(self, operation_result: Dict, processed_input: Dict) -> Any:
        ...
        if isinstance(result, dict):
            ...
            if 'recommendations' in result and not result['recommendations']:
                result['recommendations'] = self._generate_default_recommendations(
                    processed_input.get("args", [])
                )
        ...
        try:
            ...
            if param_name in self.process_limits:
                ...
                if not (limits["min"] <= numeric_value <= limits["max"]):
                    print(f"⚠️  Warning: {param_name} {numeric_value} {limits['unit']} "
            ...
            print(f"⚠️  Warning: Invalid {param_name} value: {value}, using default 0.0")
            return 0.0
    ...
    def _safety_interlock_check(self, params: Dict):
        ...
        if power > 1500 and pressure < 1.0:
            raise ValueError(
                f"Safety interlock: High power ({power}W) with low pressure ({pressure} Torr) "
                "may cause plasma instability"
            )
        
        # Very high temperature requires special handling
        if temperature > 300:
            print(f"⚠️  High temperature warning: {temperature}°C requires enhanced monitoring")
        
        # Low gas flow with high power can damage chamber
        if gas_flow_rate < 10 and power > 1000:
            raise ValueError(
                f"Safety interlock: Low gas flow ({gas_flow_rate} sccm) with high power "
                f"({power}W) may damage chamber"
            )
