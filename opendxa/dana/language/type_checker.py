...
class TypeCheckVisitor(ASTVisitor):
    ...
    def visit_assignment(self, node: Assignment, context=None) -> DanaType:
        ...
        if node.target.name in self.env:
            ...
            if not self._are_types_compatible(existing_type, value_type):
                self.errors.append(
                    TypeError(
                        f"Type mismatch in assignment to '{node.target.name}': "
                        f"expected {existing_type.value}, got {value_type.value}",
                        node.location
                    )
                )
    ...
    def visit_conditional(self, node: Conditional, context=None) -> DanaType:
        ...
        if condition_type != DanaType.BOOL and condition_type != DanaType.ANY:
            self.errors.append(
                TypeError(
                    f"Condition must be a boolean, got {condition_type.value}",
                    node.location
                )
            )
    ...
    def visit_binary_expression(self, node: BinaryExpression, context=None) -> DanaType:
        ...
        if node.operator in [BinaryOperator.ADD, BinaryOperator.SUBTRACT, BinaryOperator.MULTIPLY, BinaryOperator.DIVIDE]:
            ...
            if left_type not in [DanaType.INT, DanaType.FLOAT, DanaType.NUMBER, DanaType.ANY]:
                self.errors.append(
                    TypeError(
                        f"Left operand of '{node.operator.value}' must be a number, got {left_type.value}",
                        node.location
                    )
                )
            ...
            if right_type not in [DanaType.INT, DanaType.FLOAT, DanaType.NUMBER, DanaType.ANY]:
                self.errors.append(
                    TypeError(
                        f"Right operand of '{node.operator.value}' must be a number, got {right_type.value}",
                        node.location
                    )
                )
            ...
            if left_type != DanaType.BOOL and left_type != DanaType.ANY:
                self.errors.append(
                    TypeError(
                        f"Left operand of '{node.operator.value}' must be a boolean, got {left_type.value}",
                        node.location
                    )
                )
            ...
            if right_type != DanaType.BOOL and right_type != DanaType.ANY:
                self.errors.append(
                    TypeError(
                        f"Right operand of '{node.operator.value}' must be a boolean, got {right_type.value}",
                        node.location
                    )
                )
            ...
            if right_type not in [DanaType.STRING, DanaType.OBJECT, DanaType.ANY]:
                self.errors.append(
                    TypeError(
                        f"Right operand of 'in' must be a string or object, got {right_type.value}",
                        node.location
                    )
                )
    ...
    def visit_identifier(self, node: Identifier, context=None) -> DanaType:
        ...
        if node.name not in self.env:
            ...
            # since those are often used as placeholders for runtime values
            if "." not in node.name:
                self.errors.append(
                    TypeError(
