...
class ErrorUtils:
    """Utility class for handling DANA language parsing errors."""
            ...
            if "=" in error_line:
                ...
                if column > equals_pos and "#" in error_line[equals_pos:]:
                    ...
                    if comment_pos > equals_pos and (comment_pos - equals_pos <= 3):
                        column = equals_pos + 1
                        error_msg = ErrorUtils.create_error_message(
                            "Missing expression after equals sign", e.line, column, error_line, "Missing expression here"
                        )
                        errors.append(ParseError(error_msg, line_num=e.line, line_content=error_line))
                        return empty_program, errors

            # Clean up the error message
            error_text = str(e)
            error_msg = ErrorUtils.create_error_message(error_text, e.line, column, error_line)
            errors.append(ParseError(error_msg, line_num=e.line, line_content=error_line))
        else:
            # Generic error without position information
            error_msg = f"Parsing error: {str(e)}"
            errors.append(ParseError(error_msg, line_num=0, line_content=lines[0] if lines else ""))
