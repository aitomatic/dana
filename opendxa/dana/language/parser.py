...
class GrammarParser(Loggable):
    ...
    def __init__(self):
        ...
        # Initialize Loggable
        super().__init__(logger_name="dana.parser")
    ...
    def parse(self, program_text: str) -> ParseResult:
        ...
        try:
            ...
            self.warning(f"Unexpected input during parsing: {str(e)}")
...
def parse(code: str, type_check: bool = None) -> ParseResult:
    ...
    parser_logger.debug(f"Pre-checking code for common syntax errors")
    ...
    for i, line in enumerate(lines):
        ...
        if "=" in line_stripped and not line_stripped.endswith(":"):
            ...
            if not rest or rest.startswith("#"):
                ...
                parser_logger.warning(f"Found syntax error: {error_msg}")
                return ParseResult(
    ...
    if do_type_check and result.is_valid:
        ...
        if type_errors:
            ...
            parser_logger.warning(f"Found {len(type_errors)} type errors")
    ...
    if result.is_valid:
        ...
        parser_logger.warning(f"Parsing completed with {len(result.errors)} errors")
