...
class BaseA2AClient(A2AClient):
    ...
    @property
    def agent_card(self):
        if self._agent_card is None or datetime.now(timezone.utc) - self._last_refresh > self._refresh_interval:
    ...
    def json_agent_card(self):
        if self._json_agent_card is None or datetime.now(timezone.utc) - self._last_refresh > self._refresh_interval:
            self.refresh_agent_card()
    ...
    async def ask_with_metadata(self, message_text: str, metadata: dict[str, any] = {}) -> str:
