...
def validate_module_as_agent(module: Any) -> bool:
    ...
    """
    logger = DXA_LOGGER.getLogger("opendxa.dana.module_validator")
    ...
    try:
        ...
        if agent_name is None:
            ...
            raise InvalidModuleError(
                "Module must have 'system:agent_name' to be used as agent. "
                "Add: system:agent_name = \"Your Agent Name\""
            )
        ...
        if 'solve' not in module_dict:
            logger.error(f"Available module keys: {list(module_dict.keys())}")
            raise InvalidModuleError(
                "Module must have a 'solve' function to be used as agent. "
                "Add: def solve(task: str) -> str: ..."
            )
        ...
        if not callable(solve_func):
            raise InvalidModuleError(
                "Module 'solve' must be a callable function"
            )
