        ...
        if isinstance(data, dict):
            for field, value in data.items():
                if field not in self.indexes:
                    self.indexes[field] = {}
                # Convert value to string for indexing if it's not hashable
                index_value = str(value) if not isinstance(value, (str, int, float, bool)) else value
                if index_value not in self.indexes[field]:
                    self.indexes[field][index_value] = []
                self.indexes[field][index_value].append(key)
        return True
