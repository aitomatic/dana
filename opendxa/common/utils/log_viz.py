...
class LLMInteractionVisualizer:
    ...
    def plot_response_times(self, save_path: str | None = None):
        ...
        response_times = df["metadata"].apply(lambda x: float(x.get("duration_ms", 0)) if isinstance(x, dict) else 0.0)
    ...
    def plot_success_rate_by_phase(self, save_path: str | None = None):
        ...
        df["phase"] = df["metadata"].apply(lambda x: x.get("phase", "unknown") if isinstance(x, dict) else "unknown")
