...
import type { LibraryItem, FolderItem, BreadcrumbItem, FolderViewState } from '@/types/library';
...
export function useFolderNavigation() {
  ...
  const breadcrumbs = useMemo((): BreadcrumbItem[] => {
    if (currentPath === '/') {
      return [{ id: 'root', name: 'Knowledge Center', path: '/', type: 'root' }];
    }

    const parts = currentPath.split('/').filter(Boolean);
    const crumbs: BreadcrumbItem[] = [
      { id: 'root', name: 'Knowledge Center', path: '/', type: 'root' },
    ];
  ...
  const getItemsInCurrentFolder = useCallback(
    (allItems: LibraryItem[]): LibraryItem[] => {
      ...
      if (currentFolderId) {
        ...
        console.log('📂 Folder items for topic', topicId, ':', folderItems.length);
        return folderItems;
      }
