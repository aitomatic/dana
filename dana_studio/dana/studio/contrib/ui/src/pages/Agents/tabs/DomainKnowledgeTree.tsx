...
const DomainKnowledgeTree: React.FC<DomainKnowledgeTreeProps> = ({ agentId, knowledgePackId }) => {
  ...
  // Refetch knowledge data when knowledgePackId changes
  useEffect(() => {
    if (agentId && knowledgePackId !== undefined) {
      console.log('[DomainKnowledgeTree] Knowledge Pack changed, refetching...', { agentId, knowledgePackId });
      fetchKnowledgeData(agentId, true); // force refetch
    }
  }, [knowledgePackId, agentId, fetchKnowledgeData]);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      // Get the root node from the domain tree
      const rootNode = domainTree?.root?.topic || 'General Purpose';
        ...
        {agentId && (
          <div className={`px-4 ${initialLoading || loading ? 'top-16' : 'top-4'}`}>
            <div className="flex gap-3 pt-4 justify-between items-center">
              ...
              <div className="flex flex-1 gap-3 items-center pt-4">
                {/* Search Input */}
