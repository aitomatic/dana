...
export const HTMLRenderer: React.FC<HTMLRendererProps> = ({
  ...
  const isButtonInActiveMessage = useCallback(
    (button: Element): boolean => {
    ...
    [hasActiveButtons, currentMessageId],
  );

  // Function to handle active option button clicks
  const handleActiveOptionClick = useCallback((button: Element) => {
    const optionText = button.textContent || '';
    const now = Date.now();

    // Check if this is a duplicate click (within 500ms of the same text)
    if (
      lastClickRef.current &&
      lastClickRef.current.text === optionText &&
      now - lastClickRef.current.timestamp < 500
    ) {
      console.log(`Duplicate click detected for "${optionText}", ignoring`);
      return;
    }

    // Update last click tracker
    lastClickRef.current = { text: optionText, timestamp: now };
  ...
  }, [isButtonInActiveMessage, handleActiveOptionClick]); // Re-run if dependencies change

  return (
    <div
      ref={containerRef}
      className={cn(
        `w-full html-renderer html-renderer-${theme} html-renderer-${backgroundContext} ${textSizeClasses}`,
        className,
      )}
