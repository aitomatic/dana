...
class LangfusePromptRepository(PromptRepositoryProtocol, LocalRepositoryMixin):
    ...
    def __init__(
        self, 
        storage_config: LangfuseStorageConfig, 
        agent: BaseAgent, 
        component: BaseWAR | None = None
    ):
        """
    ...
    def _get_langfuse_prompt_name(self) -> str:
        ...
        if self._component is None:
            ...
            return f"{base_path}/{subfolder}/{self._component.__class__.__qualname__}"
    ...
    def create_snapshot(self, content: str, provenance: dict, metrics: dict) -> PromptVersionSnapshot:
        ...
        try:
            ...
            logger.info(
                f"Created prompt snapshot {version} for {self._prompt_name} "
                f"in Langfuse"
            )
