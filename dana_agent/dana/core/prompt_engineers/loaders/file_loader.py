...
"""
FileLoader is a concrete implementation of AbstractLoader that loads prompts from the local file system.
"""
...
class FileLoader(AbstractLoader):
    ...
    def get_xml(self) -> str:
        return self._load_inherited_prompt_content()

    @override
    def write_back(self, xml_string: str) -> None:
        filename = self._get_or_create_prompt_file_path(self._component.__class__.__name__)
        if filename:
    ...
    def _get_or_create_prompt_file_path(self, class_name: str) -> str:
        """
        Get existing prompt file path, or determine path for new file.
        
        First tries to find existing file using _get_co_located_prompt_file().
        If not found, returns path for new file in prompts/ subdirectory.
        
        Args:
            class_name: Name of the class to find/create prompt file for
            
        Returns:
            Path to existing or new prompt file
        """
        # First try to find existing file
        existing_file = self._get_prompt_file(class_name)
        if existing_file:
    ...
    def _get_prompt_file(self, class_name: str) -> str:
        module_name = self._component.__class__.__module__
        module = sys.modules[module_name]
        module_file = module.__file__
        if module_file is None:
            raise ValueError("Module file not found")
        prompts_folder = Path(module_file).parent.parent / "prompts"
        prompts_folder.mkdir(parents=True, exist_ok=True)
        prompt_file = prompts_folder / f"{class_name}.xml"
