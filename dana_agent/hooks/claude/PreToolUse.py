...
def query_memories(query: str, config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    # Use dana-internal's uv environment to ensure dependencies are available
    # Update this path if dana-internal is installed elsewhere
    dana_internal = os.path.expanduser("~/src/aitomatic/dana-internal/dana_agent")

    cmd = [
        "uv", "run", "--project", dana_internal,
        "dana-memory",
        "query",
