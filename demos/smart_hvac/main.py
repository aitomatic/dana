...
# Serve static files
app.mount("/static", StaticFiles(directory="demos/smart_hvac/static"), name="static")
...
class HVACDemoManager:
    ...
    async def update_systems(self):
        ...
        basic_comfort = calculate_comfort_score(
            current_temp=basic_state['temperature'],
            target_temp=self.basic_target,
            temp_history=self.basic_room.get_recent_temps()
        )
        
        # Update smart HVAC system
        smart_command = smart_hvac_control(
        ...
        smart_comfort = calculate_comfort_score(
            current_temp=smart_state['temperature'],
            target_temp=self.smart_target,
            temp_history=self.smart_room.get_recent_temps()
        )
    ...
    async def _broadcast_state(self):
        ...
        try:
            if hasattr(smart_hvac_control, '_poet_executor'):
                poet_status = smart_hvac_control._poet_executor.get_learning_status()
        except:
            pass
        ...
        message = {
            ...
            'timestamp': datetime.now().isoformat(),
            'systems': {
                'basic': self._get_system_state('basic'),
                'smart': self._get_system_state('smart')
            },
            'poet_status': poet_status,
            'metrics': {
                'basic': self._get_recent_metrics('basic'),
                'smart': self._get_recent_metrics('smart')
            }
        }
    ...
    def _get_system_state(self, system: str) -> Dict[str, Any]:
        ...
        comfort = calculate_comfort_score(
            current_temp=room.state.temperature,
            target_temp=target,
            temp_history=room.get_recent_temps()
        )
    ...
    def handle_user_input(self, user_input: UserInput):
        ...
        if user_input.system_id == 'basic':
            ...
            if user_input.action in ['too_hot', 'too_cold', 'comfortable']:
                ...
                current_temp = self.smart_room.state.temperature
                self.smart_target = self.comfort_controller.process_feedback(
                    feedback=user_input.action,
...
async def get_index():
    ...
    with open("demos/smart_hvac/static/index.html", "r") as f:
