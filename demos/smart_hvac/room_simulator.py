...
class RoomSimulator:
    ...
    def step(self, hvac_command: HVACCommand, duration: float = 1.0) -> Dict[str, Any]:
        ...
        for metric_list in [self.metrics.temperatures, self.metrics.energy_usage, 
                           self.metrics.hvac_commands]:
    ...
    def _update_weather(self):
        ...
        self.state.outside_temp = base_temp + daily_range * daily_factor * 0.5
        
        # Add weather noise
        weather_noise = random.gauss(0, 2.0)
        self.state.outside_temp += weather_noise
        
        # Update occupancy (simple pattern)
        if 6 <= self.time_of_day <= 8 or 17 <= self.time_of_day <= 23:
            self.state.occupancy = True
        else:
            self.state.occupancy = random.random() > 0.7  # 30% chance when away
