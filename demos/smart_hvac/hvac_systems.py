...
# Import POET decorator - with fallback for demo
try:
    from opendxa.dana.poet import poet
    POET_AVAILABLE = True
except ImportError:
    # Fallback: create a dummy decorator that does nothing
    print("⚠️  POET not available - using dummy decorator for demo")
    def poet(*args, **kwargs):
        def decorator(func):
            return func
        return decorator
    POET_AVAILABLE = False
...
class ComfortBasedController:
    ...
    def _adjust_learning_rate(self):
        ...
        oscillations = sum(1 for i in range(1, len(recent)) 
                          if recent[i]['feedback'] != recent[i-1]['feedback'] 
                          and recent[i]['feedback'] != 'comfortable')
