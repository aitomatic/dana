#!/bin/bash
#
# setup.sh: Unified Ollama installer + Dana configuration helper (Linux)
#
# This script replaces install.sh and start.sh by guiding you through:
#   • Installing Ollama (if missing)
#   • Choosing where Dana should store Ollama settings
#   • Configuring local LLM and/or embedding models
#   • Writing the correct variables to your chosen .env file(s)
#   • Running minimal smoke tests to confirm the setup
#
# Usage:
#   bash ./bin/ollama/setup.sh
#
...
install_ollama_linux() {
    ...
    if ! command -v curl >/dev/null 2>&1; then
        echo -e "${YELLOW}curl is required. Installing via apt-get...${NC}"
        if command -v apt-get >/dev/null 2>&1; then
            ...
            echo -e "${RED}Unable to install curl automatically. Please install curl manually and re-run this script.${NC}"
            exit 1
        fi
    fi

    if curl -fsSL https://ollama.com/install.sh | sh; then
        echo -e "${GREEN}✅ Ollama installation script completed.${NC}"
    else
        echo -e "${RED}❌ Ollama installation failed.${NC}"
        echo -e "${YELLOW}Please review the installer output above and try again.${NC}"
        exit 1
    fi
}
...
verify_ollama() {
    ...
    if [[ -z "$version" ]]; then
        ...
        echo -e "${YELLOW}Please ensure the Ollama service is properly installed and available in your PATH.${NC}"
        exit 1
    fi
...
# --- Scope selection -------------------------------------------------------
choose_env_targets() {
    echo -e"${BLUE} Now let's get you set up! ${NC}"
    echo "Dana needs to know where to store your Ollama configuration."
    ...
    echo -e "${YELLOW}We'll update: ${ENV_FILES[*]}${NC}"
    echo
}
...
select_installed_model() {
    ...
    echo -e "${GREEN}Installed models detected:${NC}"
    echo "Please enter the number corresponding to the model you want to use."
...
configure_language_model() {
    ...
    read -r -p "LOCAL_API_KEY (Press ENTER key if you don't use or require an api key.): " LM_API_KEY
    LM_API_KEY=${LM_API_KEY:-no_key_needed}

    local default_base="http://localhost:11434/v1"
    read -r -p "Base URL for OpenAI-compatible endpoint [Press ENTER to use default: ${default_base}]: " LM_BASE_URL
...
choose_embedding_model() {
    echo -e "${BLUE}We recommend 1 of the 3 embedding models below.:${NC}"
...
configure_embeddings() {
    ...
    read -r -p "Enter embedding base URL [or, press ENTER to use ollama default:${default_base}]: " EMBED_BASE_URL
    EMBED_BASE_URL=${EMBED_BASE_URL:-$default_base}

    read -r -p "Embedding batch size (Press ENTER for 32, or raise if your system can handle it and you'd like better performance): " EMBED_BATCH_SIZE
...
# --- Main ------------------------------------------------------------------
echo -e "${BLUE} Welcome to the local Ollama setup for Dana! ${NC}"
echo "This script will guide you through installation & configuration to enable usage of local LLMs and embeddings."
verify_ollama
require_command python3 "sudo apt-get install python3"
choose_env_targets
choose_usage_mode
...
echo -e "You can now run ${YELLOW}dana repl${NC} and call ${YELLOW}set_model(\"local:phi3:mini\")${NC} to set the model to use the phi3:mini model in the repl, for example."
echo -e "Afterwards, run ${YELLOW}agent Neo${NC} then ${YELLOW}Neo.chat(\"What did The Oracle teach you?\")${NC} to test it out!"