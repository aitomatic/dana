...
from dana_lang.registry.instance_registry import StructRegistry
...
class WorkflowRegistry(StructRegistry["WorkflowInstance"]):
    ...
    def get_workflows_by_status(self, status: str) -> dict[str, WorkflowInstance]:
        ...
        """
        return {
            instance_id: workflow for instance_id, workflow in self._instances.items() if self._workflow_status.get(instance_id) == status
        }
