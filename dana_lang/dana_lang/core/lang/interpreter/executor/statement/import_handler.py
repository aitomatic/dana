...
from dana_lang.core.lang.ast import ExportStatement, ImportFromStatement, ImportStatement
...
class ImportHandler(Loggable):
    ...
    def _execute_dana_import(self, module_name: str, context_name: str, context: SandboxContext) -> None:
        ...
        from __init__.init_modules import get_module_loader, get_module_registry
    ...
    def _execute_dana_from_import(
        ...
        Example:
            ``from dana_lang.core.math import add, sub as subtract``  -> binds ``local:add`` and ``local:subtract``
            ``from dana_lang.libs.corelib.na_modules import *``  -> imports all exported names
        """
        self._ensure_module_system_initialized(context)
    ...
    def _ensure_module_system_initialized(self, context: SandboxContext | None = None) -> None:
        ...
        from __init__.init_modules import get_module_loader, initialize_module_system
        ...
        for name, alias in names:
            ...
            if callable(obj) and self.function_registry:
                ...
                if crosswire_dana_functions:
                    try:
                        from dana_lang.core.lang.interpreter.functions.dana_function import DanaFunction  # type: ignore
