...
from frameworks.knows.core.knowledge_orgs.base import KnowledgeOrganization, QueryError, StorageError
from frameworks.knows.core.knowledge_orgs.config import VectorStoreSettings
    ...
    def retrieve(self, key: str) -> dict[str, Any] | None:
        ...
        try:
            with self.conn.cursor() as cur:
                ...
                return {"embedding": self._parse_vector(result[0]), "metadata": result[1]}
        except psycopg2.Error as e:
    ...
    def query(self, **kwargs) -> list[Any]:
        ...
        try:
            ...
            with self.conn.cursor() as cur:
                ...
                return [
                    {"id": row[0], "embedding": self._parse_vector(row[1]), "metadata": row[2], "similarity": row[3]} for row in results
                ]
