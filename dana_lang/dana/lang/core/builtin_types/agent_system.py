...
from dana.lang.core.builtin_types.struct_system import StructInstance, StructType
...
from dana.lang.core.concurrency.promise_utils import is_promise
from dana.lang.core.lang.sandbox_context import SandboxContext

# For backward compatibility, create aliases
from dana.lang.registry import (
    get_agent_type,
)
...
def default_solve_method(
    ...
    workflows: list[WorkflowInstance] | None = None,
) -> Any:
    ...
    def wrapper():
        return agent_instance._solve_impl(
            sandbox_context=sandbox_context, problem=problem, context=context, resources=resources, workflows=workflows
        )
...
class AgentInstance(StructInstance):
    ...
    def __init__(self, struct_type: AgentType, values: dict[str, Any]):
        ...
        from dana.lang.registry import AGENT_REGISTRY
    ...
    def solve(
        ...
        if method:
            ...
            return method(self, sandbox_context=sandbox_context, problem=problem, context=context, resources=resources, workflows=workflows)
        else:
            # Fallback to built-in solve implementation
            return default_solve_method(
                self, sandbox_context=sandbox_context, problem=problem, context=context, resources=resources, workflows=workflows
            )
    ...
    def reason(
        self, sandbox_context: SandboxContext, premise: str, context: dict | None = None, resources: list[ResourceInstance] | None = None
    ) -> Any:
        ...
        if method:
            ...
            return method(self, sandbox_context=sandbox_context, premise=premise, context=context, resources=resources)
        else:
            # Fallback to built-in reason implementation
            return default_reason_method(self, sandbox_context=sandbox_context, premise=premise, context=context, resources=resources)
    ...
    def _initialize_conversation_memory(self):
        ...
        if self._conversation_memory is None:
            ...
            from dana.lang.frameworks.memory.conversation_memory import ConversationMemory
    ...
    def _plan_impl(self, sandbox_context: SandboxContext, task: str, context: dict | None = None) -> str:
        ...
        try:
            from dana.lang.libs.corelib.py_wrappers.py_reason import py_reason
