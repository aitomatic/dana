...
class GenerateAdditionalQuestionsTool(BaseTool):
    ...
    async def _execute(self, topic_name: str, focus_area: str = "", num_questions: int = 3, **kwargs) -> ToolResult:
        """
        ...
        try:
            ...
            if not knowledge_summary:
                return ToolResult(
                    name="generate_additional_questions",
                    result=f"❌ No knowledge summary found for topic '{topic_name}'. Make sure knowledge generation has been completed for this topic.",
                    require_user=False,
                )
            ...
            if tasks:
                ...
                result_idx = 0
                if self.rag_knows:
                    knows_results = results[result_idx]
                    knows_context = "\n\n".join([r.text for r in knows_results[:5]])
                    result_idx += 1

                if self.rag_docs:
                    docs_results = results[result_idx]
                    docs_context = "\n\n".join([r.text for r in docs_results[:5]])

            # Generate questions using LLM with RAG context
            generated_questions = await self._generate_questions_from_summary(
                topic_name, knowledge_summary, focus_area, num_questions, self.domain, self.role, knows_context, docs_context
            )
            ...
            # Format result
            content = self._format_generated_questions(topic_name, generated_questions, focus_area)
    ...
    async def _find_knowledge_summary(self, topic_name: str, knowledge_pack_path: str) -> str:
        ...
        try:
            ...
            knows_dir = knowledge_pack_dir / "knows"
    ...
    def _format_generated_questions(self, topic_name: str, questions: list, focus_area: str) -> str:
