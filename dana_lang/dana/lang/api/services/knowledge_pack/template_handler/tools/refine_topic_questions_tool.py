...
from ..utils import parse_template, find_topic_by_name, write_template
...
class RefineTopicQuestionsTool(BaseTool):
    ...
    async def _execute(self, topic_name: str, refinement_instruction: str, preserve_existing: bool = True, **kwargs) -> ToolResult:
        ...
        try:
            ...
            # Find the topic
            topic = find_topic_by_name(template_data["topics"], topic_name)
            if not topic:
                return ToolResult(
                    name="refine_topic_questions",
                    result=f"❌ Topic '{topic_name}' not found in template",
                    require_user=False,
                )
