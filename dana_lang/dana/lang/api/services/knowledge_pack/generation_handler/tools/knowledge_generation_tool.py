...
from dana.lang.api.repositories.config import DEFAULT_TEMPLATE_FOLDER
import logging
import asyncio
import re
...
class KnowledgeGenerationTool(BaseTool):
    def __init__(
        ...
        notifier: Callable[[str, str, Any, float | None], Awaitable[None]] | None = None,
    ...
    async def _execute(self, user_message: str = "", counts: str = "", context: str = "", **kwargs) -> ToolResult:
        ...
        3. Generate master interview template (reads all summaries and creates template)
        ...
        try:
            ...
            try:
                logger.info("🎯 Step 3: Generating master interview template...")
                await self._generate_master_interview_template()
                logger.info("✅ Master interview template generated successfully")
            except Exception as e:
    ...
    async def _generate_knowledge_for_all_leaves(self, user_message: str, counts: str, context: str) -> None:
        ...
        if self.notifier:
            await self.notifier(
                "generate_knowledge", f"🌳 Starting knowledge generation for {len(all_leaf_paths)} topics", "in_progress", 0.0
            )
        ...
        if self.notifier:
            await self.notifier("generate_knowledge", f"✅ Knowledge generation complete. Summary: \n{status_text}", "finish", 1.0)
...
"""
    ...
    async def _generate_master_interview_template(self) -> None:
        """
        Generate master interview template by discovering and reading all knowledge.json files with summaries.
...
## Interview Approach
- **Goal**: Capture expert's natural knowledge flow
- **Style**: Conversational, expert-driven
- **Duration**: 60-90 minutes total
- **Topics Covered**: {len(all_summaries_data)} topics
...
**Listen for connections to**: [Related topics based on key concepts]
