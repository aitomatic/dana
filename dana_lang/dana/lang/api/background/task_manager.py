...
from dana.lang.api.repositories import (
...
from dana.lang.api.core.schemas import ExtractionDataRequest
from dana.lang.api.core.schemas_v2 import KnowledgeGenerationStatus, TemplateGenerationStatus
from dana.lang.api.services.extraction_service import get_extraction_service
...
from dana.lang.api.core.schemas_v2 import BackgroundTaskType
...
class TaskManager:
    ...
    def _load_pending_tasks(self):
        ...
        try:
            for db in get_db():
                ...
                from dana.lang.api.core.schemas_v2 import BackgroundTaskStatus
    ...
    def process_task(self, task: dict):
        ...
        try:
            ...
            if task_id:
                from dana.lang.api.core.schemas_v2 import BackgroundTaskStatus
                from dana.lang.api.routers.v2.ws.domain_knowledge_ws import kp_generation_ws_notifier
            ...
            if task["type"] == BackgroundTaskType.KNOWLEDGE_GEN:
                ...
                tree_structure = Misc.safe_asyncio_run(self.kp_cls.get_kp_tree, task["data"]["knowledge_id"])
                ...
                knowledge_gen_tool = KnowledgeGenerationTool(
                    ...
                    notifier=kp_generation_ws_notifier.get_notifier(task["data"]["knowledge_id"]),
                )
                ...
                if "knowledge_id" in task["data"]:
                    for db in get_db():
                        Misc.safe_asyncio_run(
                            self.kp_cls.update_kp,
                            task["data"]["knowledge_id"],
                            kp_metadata={},
                            status=KnowledgeGenerationStatus.COMPLETED,
                            db=db,
                        )

                # Update master template is_active to True after knowledge generation completes
                try:
                    ...
                    for db in get_db():
                        ...
                        if master_template:
                            ...
                            update_data = InterviewTemplateUpdate(
                                is_active=True, template_metadata={"status": TemplateGenerationStatus.COMPLETED}
                            )
                            ...
                            logger.info(f"Updated master template {master_template.id} is_active to True for knowledge pack {knowledge_id}")
                        else:
                            logger.warning(f"No master template found for knowledge pack {knowledge_id}")

                except Exception as e:
                    logger.error(f"Failed to update master template for knowledge pack {task['data']['knowledge_id']}: {e}")
    ...
    def _process_deep_extract_task(self, task: dict):
        ...
        try:
            ...
            from dana.lang.api.routers.v1.extract_documents import deep_extract
            from dana.lang.api.core.schemas import DeepExtractionRequest
    ...
    def _update_task_status(self, task_id: int, status, error: str | None = None):
        ...
        try:
            from dana.lang.api.core.models import BackGroundTask
