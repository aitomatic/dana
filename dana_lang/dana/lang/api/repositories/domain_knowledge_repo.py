...
from dana.lang.api.core.models import KnowledgePack, Document
from dana.lang.api.core.schemas import DomainNode
...
import shutil
import logging
from dana.lang.api.repositories.config import DEFAULT_TEMPLATE_FOLDER, DOMAIN_TREE_FN
...
class SQLDomainKnowledgeRepo(AbstractDomainKnowledgeRepo):
    ...
    async def _create_default_interview_template(cls, kp_id: int, kp_metadata: dict, **kwargs) -> None:
        ...
        """
        from dana.lang.api.repositories import get_interview_template_repo
        from dana.lang.api.core.schemas_v2 import InterviewTemplateCreate
        ...
        readme_file.write_text("# Master Interview Template\n\n*Template will be generated after knowledge generation completes.*\n")

        # Create database record
        template_data = InterviewTemplateCreate(
            kp_id=kp_id,
            name=f"Master Interview Template - {domain} {role}",
            description=f"Primary interview template for {role} in {domain}",
            version="1.0.0",
            folder_path=str(master_template_folder.resolve()),
            is_active=False,
            is_master=True,
            template_metadata={
                "domain": domain,
