...
from dxa.execution.reasoning.reasoning_factory import ReasoningFactory
from dxa.execution.reasoning.reasoning import Reasoning
...
class TestReasoningFactory:
    ...
    def test_create_default_reasoning_from_config(self):
        """Test creating a reasoning from config."""
        objective = Objective("Test objective")
        reasoning = ReasoningFactory.create_from_config("default", objective)
        
        assert isinstance(reasoning, Reasoning)
        assert reasoning.objective == objective
        assert len(reasoning.nodes) >= 3  # At least START, one task, and END
        
        # Check that we have task nodes
        task_nodes = [node for node in reasoning.nodes.values()
                      if node.node_type == NodeType.TASK]
        assert len(task_nodes) > 0
        
        # Check that we have START and END nodes
        assert any(node.node_type == NodeType.START for node in reasoning.nodes.values())
        assert any(node.node_type == NodeType.END for node in reasoning.nodes.values())
        
        # Check that edges connect all nodes
        for node_id in reasoning.nodes:
            if node_id != "END":  # END should have no outgoing edges
                assert reasoning.get_outgoing_edges(node_id)
    ...
    def test_node_structure_and_metadata(self):
        ...
        expected_nodes = ["START", "UNDERSTAND", "EXPLORE", "SOLVE", "REVIEW", "END"]
        for node_id in expected_nodes:
            assert node_id in reasoning.nodes, f"Missing expected node: {node_id}"
    ...
    def test_edge_connections(self):
        ...
        while current != "END":
            visited.add(current)
            outgoing = reasoning.get_outgoing_edges(current)
        ...
        all_nodes = set(reasoning.nodes.keys())
    ...
    def test_role_setting(self):
        ...
        reasoning = ReasoningFactory.create_from_config("default", objective, role=role)
        
        # Check that the role is set in metadata
        assert reasoning.metadata.get("role") == role, "Role not properly set in metadata"
