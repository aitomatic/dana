"""
Test runner for Dana syntax limitation tests.

This module discovers and runs .na test files in the syntax_limitations directory,
marking them as expected failures in pytest. These tests document Dana language
features that seem natural but aren't currently supported.
"""
...
@pytest.mark.xfail(reason="Expected failure: Documents unsupported Dana syntax features for future consideration")
def test_expected_syntax_failures(dana_file):
    """
    Test expected syntax failures in Dana language.
    
    These tests are marked as expected failures (xfail) because they document
    language features that don't work yet but might be supported in the future.
    
    The .na files are designed to:
    1. Try syntax features that should fail
