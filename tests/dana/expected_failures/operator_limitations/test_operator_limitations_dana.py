"""
Test runner for Dana operator limitation tests.

This module discovers and runs .na test files in the operator_limitations directory,
marking them as expected failures in pytest. These tests document operator features
...
@pytest.mark.xfail(reason="Expected failure: Documents unsupported Dana operator features for future consideration")
def test_expected_operator_failures(dana_file):
    """
    Test expected operator failures in Dana language.
    
    These tests are marked as expected failures (xfail) because they document
    operator features that don't work yet but might be supported in the future.
    
    The .na files are designed to:
    1. Try operator features that should fail
