...
def test_assignment(dana_parser):
    ...
    # Assignment target should be a variable (implicit local), scoped_var, etc.
    target = assignment.children[0]
    # target.data == 'target', so check its first child
...
def test_assignment_indexing(dana_parser):
    ...
    assert assignment is not None
    target = assignment.children[0]
    # For arr[0] = 5, target should have children: atom and expr
...
def test_string_and_literals(dana_parser):
    ...
    for i in range(3):
        expr = assignments[i].children[1]  # assignment: target '=' expr
        atom = find_first(expr, "atom")
        assert atom is not None
        # For a literal, atom.children should be empty
...
def test_scope_and_indexing(dana_parser):
    ...
    # For arr[0], look for a trailer node with an expr child
...
def test_fstring_assignment(dana_parser):
    ...
    assignment = find_first(tree, "assignment")
    assert assignment is not None
    expr = assignment.children[1]  # assignment: target '=' expr
    atom = find_first(expr, "atom")
    assert atom is not None
    string_node = find_first(atom, "f_string")
    assert string_node is not None
    # Check that the f-string is present as a token or child
    fstring_found = any(
