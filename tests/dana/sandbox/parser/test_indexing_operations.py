...
"""
        ...
        assert not isinstance(result, pd.core.indexing._iLocIndexer), f"Should not return indexer object, got {type(result)}"

        # Check that result is a pandas Series (the expected return type)
        assert isinstance(result, pd.Series), f"Expected pandas Series, got {type(result)}"
