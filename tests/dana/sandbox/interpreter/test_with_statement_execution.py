...
def mock_use_function():
    ...
    def mock_use(context, function_name, *args, _name=None, **kwargs):
        if function_name.lower() == "mcp":
            resource = MockMCPResource(_name or "test_resource", *args, **kwargs)
            if _name:
    ...
    with patch('opendxa.dana.sandbox.interpreter.functions.core.use_function.use_function', mock_use):
        yield mock_use
