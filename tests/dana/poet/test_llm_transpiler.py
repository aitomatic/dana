...
class TestLLMTranspiler:
    ...
    def test_llm_transpiler_context_extraction(self):
        ...
        with patch.object(transpiler, 'llm') as mock_llm:
            mock_response = {
                "choices": [{
                    "message": {
                        "content": "# Generated Dana code would go here"
                    }
                }]
            }
    ...
    def test_llm_domain_specific_prompts(self):
        ...
        assert "compliance" in financial_prompt
        assert "fraud" in financial_prompt
        ...
        assert "retry" in api_prompt.lower()
        assert "cache" in api_prompt.lower()
...
'''
    ...
    def test_different_domains_generate_different_code(self):
        ...
        with patch.object(transpiler, 'llm') as mock_llm:
            ...
            mock_llm.query_sync.return_value = {
                "choices": [{
                    "message": {"content": "# Code with caching logic"}
                }]
            }
            ...
            mock_llm.query_sync.return_value = {
                "choices": [{
                    "message": {"content": "# Code with connection pooling"}
                }]
            }
...
class TestLLMGenerationQuality:
    ...
    @pytest.mark.skipif(
        not Path(".env").exists() or "OPENAI_API_KEY" not in open(".env").read(),
        reason="Requires API keys for live LLM testing"
    )
