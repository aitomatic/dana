...
from opendxa.knows.extraction.meta.categorizer import (
...
class TestMetaKnowledgeExtractor:
    ...
    def test_extract_response_text_openai_format(self):
        ...
        response = {
            "choices": [{
                "message": {
                    "content": "Extracted knowledge point"
                }
            }]
        }
        result = self.extractor._extract_response_text(response)
    ...
    @patch('opendxa.knows.extraction.meta.extractor.MetaKnowledgeExtractor._extract_with_llm')
    def test_process_success(self, mock_extract):
        ...
        mock_kp = KnowledgePoint(
            id="kp_1",
            type="process",
            content="Temperature monitoring process",
            context={"test": True},
            confidence=0.8,
            metadata={}
        )
    ...
    def test_process_with_filtering(self, mock_extract):
        ...
        mock_kps = [
            KnowledgePoint(id="kp_1", type="process", content="High confidence", 
                          context={}, confidence=0.9, metadata={}),
            KnowledgePoint(id="kp_2", type="fact", content="Low confidence", 
                          context={}, confidence=0.3, metadata={})  # Below threshold
        ]
    ...
    def test_process_with_limit(self, mock_extract):
        ...
        mock_kps = [
            KnowledgePoint(id=f"kp_{i}", type="process", content=f"Content {i}", 
                          context={}, confidence=0.8, metadata={})
...
class TestKnowledgeCategorizer:
    ...
    def test_have_contextual_relationship_same_document(self):
        ...
        kp1 = KnowledgePoint(
            id="kp1", type="process", content="Test 1",
            context={"source_document_id": "doc_1"},
            confidence=0.8, metadata={}
        )
        kp2 = KnowledgePoint(
            id="kp2", type="fact", content="Test 2", 
            context={"source_document_id": "doc_1"},
            confidence=0.8, metadata={}
        )
    ...
    def test_have_contextual_relationship_false(self):
        ...
        kp2 = KnowledgePoint(
            id="kp2", type="fact", content="Test 2", 
            context={"source_document_id": "doc_2"},
            confidence=0.8, metadata={}
        )
