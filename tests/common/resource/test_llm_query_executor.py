...
class TestLLMQueryExecutor(unittest.TestCase):
    """Test the LLMQueryExecutor class."""
    ...
    def test_set_mock_llm_call(self):
        ...
        with self.assertRaises(LLMError):
            self.query_executor.set_mock_llm_call("invalid")
    ...
    async def test_mock_llm_query(self):
        ...
        self.assertIn("Mock response to: Hello, how are you?", response["choices"][0]["message"]["content"])
    ...
    async def test_query_once_success(self):
        ...
        mock_response.choices = [{"message": {"role": "assistant", "content": "Test response"}}]
        ...
        build_request_params = MagicMock(return_value={
            "messages": [{"role": "user", "content": "test"}],
            "model": "openai:gpt-4",
            "temperature": 0.7
        })
    ...
    async def test_query_iterative_basic(self):
        ...
        mock_build_params = MagicMock(return_value={
            "messages": [], "model": "openai:gpt-4", "temperature": 0.7
        })
        ...
        self.query_executor.query_once = AsyncMock(return_value=mock_response)
        ...
        response = await self.query_executor.query_iterative(
            request, mock_tool_handler, mock_build_params
        )
        
        self.assertEqual(response, mock_response)
        self.query_executor.query_once.assert_called_once()

    async def test_query_iterative_with_tool_calls(self):
        """Test query_iterative with tool calls."""
        # Set up mock tool call handler
        mock_tool_handler = AsyncMock(return_value=[
            {"role": "tool", "content": "Tool response", "tool_call_id": "test_id"}
        ])
        ...
        self.query_executor.query_once = AsyncMock(side_effect=[response_with_tools, response_final])
        self.query_executor.query_strategy = QueryStrategy.ITERATIVE
        self.query_executor.query_max_iterations = 3
        
        request = {
            "user_messages": [{"role": "user", "content": "Hello"}],
            "available_resources": {"test_resource": MagicMock()}
        }
        ...
        self.assertEqual(response, response_final)
