...
class TestCommaSeparatedImports:
    ...
    def test_basic_comma_separated_import(self):
        ...
        result = self.sandbox.eval("from math.py import sqrt, pi")
        ...
        pi_result = self.sandbox.eval("pi")
    ...
    def test_comma_separated_import_with_aliases(self):
        ...
        result = self.sandbox.eval("from json.py import dumps as jd, loads as jl")

        assert result.success is True
        assert result.result is None

        # Test that aliased functions are accessible
        dumps_result = self.sandbox.eval('jd({"test": 123})')
        loads_result = self.sandbox.eval('jl(\'{"key": "value"}\')')
    ...
    def test_mixed_comma_separated_import(self):
        ...
        result = self.sandbox.eval("from math.py import sqrt, pi as PI, floor")

        assert result.success is True
        assert result.result is None

        # Test all imported names
        sqrt_result = self.sandbox.eval("sqrt(25)")
        pi_result = self.sandbox.eval("PI")
        floor_result = self.sandbox.eval("floor(3.7)")
    ...
    def test_three_comma_separated_imports(self):
        ...
        result = self.sandbox.eval("from math.py import sqrt, pi, floor")

        assert result.success is True
        assert result.result is None

        # Test all three imported names
        sqrt_result = self.sandbox.eval("sqrt(9)")
        pi_result = self.sandbox.eval("pi")
        floor_result = self.sandbox.eval("floor(2.9)")
    ...
    def test_comma_separated_import_error_handling(self):
        ...
        result = self.sandbox.eval("from math.py import sqrt, nonexistent_function")
    ...
    def test_comma_separated_import_with_single_alias(self):
        ...
        result = self.sandbox.eval("from math.py import sqrt as square_root")

        assert result.success is True
        assert result.result is None

        # Test that the aliased name is accessible
        sqrt_result = self.sandbox.eval("square_root(16)")
        assert sqrt_result.success is True
        assert sqrt_result.result == 4.0
    ...
    def test_submodule_import_2_components(self):
        ...
        try:
            result = self.sandbox.eval("from dana.frameworks import workflow, agent")
            # If the module exists, test functionality
            if result.success:
                # Test that the imported modules are accessible
                workflow_result = self.sandbox.eval("workflow")
                agent_result = self.sandbox.eval("agent")
    ...
    def test_submodule_import_3_components(self):
        ...
        try:
            result = self.sandbox.eval("from dana.frameworks.workflow import core, engine")
            # If the module exists, test functionality
            if result.success:
                # Test that the imported modules are accessible
                core_result = self.sandbox.eval("core")
                engine_result = self.sandbox.eval("engine")
    ...
    def test_submodule_import_3_components_with_aliases(self):
        ...
        try:
            result = self.sandbox.eval("from dana.frameworks.workflow import core as wf_core, engine as wf_engine")
            # If the module exists, test functionality
            if result.success:
                # Test that the aliased modules are accessible
                core_result = self.sandbox.eval("wf_core")
                engine_result = self.sandbox.eval("wf_engine")
                assert core_result.success is True
                assert engine_result.success is True
            else:
                # Module not found is acceptable for this test
    ...
    def test_submodule_import_grammar_parsing(self):
        ...
        for test_case in test_cases:
            try:
                result = self.sandbox.eval(test_case)
