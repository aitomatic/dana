...
def _time_to_target_minutes(T0: float, T_target: float, Tinf: float, tau_min: float) -> float:
    ...
    if ratio <= 0:
        ...
        print(
            f"      Fallback calculation: {C_to_F(T0):.1f}°F → {C_to_F(T_target):.1f}°F, diff={temp_diff_f:.1f}°F, time={time_estimate:.0f}min"
        )
...
def check_hvac_schedule(
    ...
    *,
    ...
    if time_needed_minutes <= time_available_minutes:
        ...
        error_msg = (
            f"Need {time_needed_minutes} min to cool down, "
...
def validate_plan_success(
    ...
    *,
    ...
    for i, (action, target_temp) in enumerate(zip(plan, target_temps, strict=False)):
        ...
        if schedule_result["reached_temp"] == "success" and meeting_start_time and schedule_result["reached_time"]:
            ...
            if reached_minutes > meeting_start_minutes:
                ...
                final_error = (
                    f"Target reached at {schedule_result['reached_time']} "
        ...
        if final_schedule_success == "failed":
            failed_actions.append(
                {"action_index": i, "time_on": time_on, "time_off": time_off, "target_temp_f": target_temp, "error": final_error}
            )
