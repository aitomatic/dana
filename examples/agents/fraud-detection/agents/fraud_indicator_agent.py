...
class BroadcastNotificationHandler(Notifiable):
    ...
    def __init__(self, agent_name: str = "FraudIndicatorAgent", verbose: bool = True):
        """
...
class FraudIndicatorAgent(STARAgent):
    ...
    def __init__(self, agent_id: str | None = None, llm_provider: str = "anthropic", model: str = "claude-3-5-sonnet-20241022", **kwargs):
        ...
        self.notification_handler = BroadcastNotificationHandler("FraudIndicatorAgent")
...
if __name__ == "__main__":
    # Test the FraudIndicatorAgent with notifications
    agent = FraudIndicatorAgent()

    # Enable notifications to see all broadcast messages
    agent.enable_notifications(verbose=True)

    print("🚀 Testing FraudIndicatorAgent Notification System")
    print("=" * 60)
    print(f"📊 Initial notification count: {agent.get_notification_count()}")

    # Test fraud detection with notification monitoring
    print("\n🔍 Starting fraud detection with notification monitoring...")
    print("=" * 60)

    try:
        # Run a simple test conversation
        result = agent.converse(initial_message="Analyze this data for fraud patterns")
