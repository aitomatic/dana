...
def matches_are_equivalent(matches1, matches2, ignore_order=True):
    ...
    for i, (m1, m2) in enumerate(zip(norm1, norm2, strict=False)):
        # Compare each field
...
def test_literal_search_consistency(search_resource_fixtures):
    ...
    python_result = resource.search(pattern=pattern, is_regex=False, is_case_sensitive=False, max_results=10, context_lines=0)

    # Force ripgrep search (if available)
    resource.ripgrep_available = True
    ripgrep_result = resource.search(pattern=pattern, is_regex=False, is_case_sensitive=False, max_results=10, context_lines=0)
...
def test_literal_search_amd_file(search_resource_amd_data):
    ...
    python_result = resource.search(
        pattern=pattern, is_regex=False, is_case_sensitive=False, file_pattern="AMD-AR.md", max_results=20, context_lines=0
    )
...
def test_regex_whitespace_pattern(search_resource_amd_data):
    ...
    assert (
        python_result["total_matches"] == ripgrep_result["total_matches"]
...
def test_path_resolution_consistency(search_resource_amd_data):
    ...
    for file_pattern in test_patterns:
        ...
        resource.ripgrep_available = False
        python_result = resource.search(
            pattern=pattern, is_regex=False, is_case_sensitive=False, file_pattern=file_pattern, max_results=5, context_lines=0
        )
        ...
        assert (
            python_result["success"] == ripgrep_result["success"]
...
def test_financial_pattern_with_context():
    ...
    resource.ripgrep_available = False
    python_result = resource.search(
        pattern=pattern, is_regex=True, is_case_sensitive=False, file_pattern=file_pattern, max_results=20, context_lines=3
    )
    ...
    if python_result["matches"] and ripgrep_result["matches"]:
        for i in range(min(3, len(python_result["matches"]), len(ripgrep_result["matches"]))):
            ...
            assert (
                abs(py_match["line_number"] - rg_match["line_number"]) <= 1
