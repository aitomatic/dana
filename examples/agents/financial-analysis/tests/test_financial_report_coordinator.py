...
"""
    ...
    result = create_resource.create(
        relative_workspace_path="reports/test_report.md",
...
def test_multiple_coordinators():
    ...
    try:
        ...
        analyst1 = FinancialAnalysisAgent(
            agent_id="analyst-1",
            workspace_root=temp_dir1,
            model="gpt-4.1-mini"
        )
        coordinator1 = FinancialReportCoordinatorAgent(
            agent_id="coordinator-1",
            workspace_root=temp_dir1,
            financial_analysis_agent=analyst1,
            model="gpt-4.1-mini"
        )
        
        # Create second coordinator
        analyst2 = FinancialAnalysisAgent(
            agent_id="analyst-2",
            workspace_root=temp_dir2,
            model="gpt-4.1-mini"
        )
        coordinator2 = FinancialReportCoordinatorAgent(
            agent_id="coordinator-2",
            workspace_root=temp_dir2,
            financial_analysis_agent=analyst2,
            model="gpt-4.1-mini"
        )
...
def test_workspace_root_variations():
    ...
    try:
        ...
        coordinator1 = FinancialReportCoordinatorAgent(
            agent_id="coord-abs-path",
            workspace_root=str(Path(temp_dir).absolute()),
            financial_analysis_agent=analyst,
            model="gpt-4.1-mini"
        )
