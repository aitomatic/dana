...
def main():
    ...
    indexer = DataFrameIndexerResource(workspace_root=str(dataset_dir), cache_dir=cache_dir, debug=True)

    metadata_extractor = MetadataExtractorResource(workspace_root=str(dataset_dir))

    # Create workflow
    workflow = TabularAnalysisWorkflow(
        dataframe_indexer=indexer, metadata_extractor=metadata_extractor, llm_provider="openai", llm_model="gpt-4o-mini"
    )
