...
# Page configuration
st.set_page_config(page_title="SMBC Credit Approval System", page_icon="🏦", layout="wide", initial_sidebar_state="expanded")
...
def chat_with_dana(user_message: str, context: dict[str, Any] | None = None) -> str:
...
def display_application_form():
    ...
    with st.form("credit_application"):
        ...
        with col1:
            ...
            employment_type = st.selectbox("Employment Type", ["permanent", "contract", "temporary", "self-employed"])
            employment_years = st.number_input("Years of Employment", min_value=0, max_value=50, value=5)
            annual_income = st.number_input("Annual Income (¥)", min_value=1000000, max_value=100000000, value=50000000, step=1000000)

        with col2:
            existing_debt = st.number_input("Existing Debt (¥)", min_value=0, max_value=50000000, value=5000000, step=100000)
            credit_history = st.selectbox("Credit History", ["excellent", "good", "fair", "poor", "none"])
            requested_amount = st.number_input(
                "Requested Loan Amount (¥)", min_value=1000000, max_value=100000000, value=30000000, step=1000000
            )
            loan_purpose = st.selectbox("Loan Purpose", ["home_purchase", "business", "education", "debt_consolidation", "other"])
...
def display_chat_interface():
    ...
    with col1:
        if st.button("🔍 Analyze Application", key="analyze_btn"):
            if st.session_state.current_application:
                ...
                st.session_state.chat_history.append({"user": "Analyze Application", "agent": response, "timestamp": datetime.now()})

    with col2:
        if st.button("⚠️ Risk Factors", key="risk_btn"):
            if st.session_state.current_application:
                response = check_risk_factors(st.session_state.current_application)
                st.session_state.chat_history.append({"user": "Check Risk Factors", "agent": response, "timestamp": datetime.now()})

    with col3:
        if st.button("📋 Compliance Check", key="compliance_btn"):
            if st.session_state.current_application:
                response = check_compliance(st.session_state.current_application)
                st.session_state.chat_history.append({"user": "Check Compliance", "agent": response, "timestamp": datetime.now()})

    with col4:
        if st.button("💡 Explain Decision", key="explain_btn"):
            if st.session_state.current_application:
                response = explain_decision(st.session_state.current_application)
                st.session_state.chat_history.append({"user": "Explain Decision", "agent": response, "timestamp": datetime.now()})
    ...
    if st.button("Send Message", key="send_btn"):
        if user_message.strip():
            ...
            st.session_state.chat_history.append({"user": user_message, "agent": "", "timestamp": datetime.now()})
...
def main():
    ...
    with col1:
        ...
        # UCI dataset loader
        case_number = st.number_input("Load UCI Case (1-690):", min_value=1, max_value=690, value=1)
