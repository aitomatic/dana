#!/usr/bin/env python3
"""
Environment state generator for HVAC simulation.

Refactored from single_room.py - exact logic preserved.
Generates fresh random environment state (not persistent).
"""

import random
from typing import Optional
from .config import EnvironmentConfig


class EnvironmentGenerator:
    """
    Generates fresh random environment state for a room.

    Each call to generate_state() produces new random values for:
    - Current time (08:00-21:59)
    - Outdoor temperature (based on time of day)
    - Indoor temperature (outdoor ± 2°F)
    - Meeting schedule (0-5 meetings with constraints)
    """

    def __init__(self, config: EnvironmentConfig, seed: Optional[int] = None):
        """
        Initialize environment generator.

        Args:
            config: Room configuration
            seed: Optional random seed for reproducibility (testing only)
        """
        self.config = config
        if seed is not None:
            random.seed(seed)

    def generate_state(self) -> dict:
        """
        Generate fresh random environment state.

        Returns:
            Dictionary with:
            - room_name: str
            - current_time: str (HH:MM format)
            - indoor_temp: float (°F)
            - outdoor_temp: float (°F)
            - meeting_plan: list of dicts with start_time and end_time
        """
        current_time = self._generate_current_time()
        outdoor_temp = self._generate_outdoor_temp(current_time)
        indoor_temp = self._generate_indoor_temp(outdoor_temp)
        meeting_plan = self._generate_meeting_plan(current_time)

        return {
            "room_name": self.config.room_name,
            "current_time": current_time,
            "indoor_temp": indoor_temp,
            "outdoor_temp": outdoor_temp,
            "meeting_plan": meeting_plan
        }

    def _generate_current_time(self) -> str:
        """
        Generate random time in HH:MM format.

        Range: 08:00-21:59 (extended business hours)
        """
        hour = random.randint(8, 21)
        minute = random.randint(0, 59)
        return f"{hour:02d}:{minute:02d}"

    def _generate_outdoor_temp(self, current_time: str) -> float:
        """
        Generate realistic outdoor temperature based on time of day.

        Simulates daily temperature cycle:
        - Coolest: 06:00-08:00 (45-65°F)
        - Morning: 09:00-11:00 (55-75°F)
        - Afternoon: 12:00-16:00 (70-95°F) - hottest
        - Evening: 17:00-19:00 (60-85°F)
        - Night: 20:00-21:00 (50-75°F)

        Args:
            current_time: Time in HH:MM format

        Returns:
            Outdoor temperature in °F
        """
        hour = int(current_time.split(':')[0])

        # Simulate daily temperature cycle
        if 6 <= hour <= 8:
            # Early morning: coolest
            base_outdoor = random.uniform(45, 65)
        elif 9 <= hour <= 11:
            # Morning: warming up
            base_outdoor = random.uniform(55, 75)
        elif 12 <= hour <= 16:
            # Afternoon: hottest
            base_outdoor = random.uniform(70, 95)
        elif 17 <= hour <= 19:
            # Evening: cooling down
            base_outdoor = random.uniform(60, 85)
        else:  # 20-21
            # Night: cooler
            base_outdoor = random.uniform(50, 75)

        # Add random variation
        variation = random.uniform(-3, 3)
        outdoor_temp = base_outdoor + variation

        return round(outdoor_temp, 1)

    def _generate_indoor_temp(self, outdoor_temp: float) -> float:
        """
        Generate indoor temperature based on outdoor temperature.

        Without HVAC running, indoor temp should be very close to outdoor temp.
        Includes small offset for building thermal mass and insulation.

        Args:
            outdoor_temp: Outdoor temperature in °F

        Returns:
            Indoor temperature in °F
        """
        # Small offset for building thermal mass and insulation (±2°F)
        thermal_offset = random.uniform(-2, 2)

        # Indoor temp = outdoor temp with small thermal offset
        base_indoor = outdoor_temp + thermal_offset

        # Add small random variation for realism (±0.5°F)
        variation = random.uniform(-0.5, 0.5)
        indoor_temp = base_indoor + variation

        return round(indoor_temp, 1)

    def _generate_meeting_plan(self, current_time_str: str) -> list[dict]:
        """
        Generate diverse meeting plan with realistic constraints.

        Constraints:
        - 0-5 meetings per day
        - Earliest meeting: at least 2 hours from current time
        - Meeting durations: 30-120 minutes (or 15-30 for late-day)
        - Gaps between meetings: 30-180 minutes
        - All meetings end before 22:00

        Args:
            current_time_str: Current time in HH:MM format

        Returns:
            List of meeting dictionaries with start_time and end_time
        """
        # Parse current time
        current_h, current_m = map(int, current_time_str.split(':'))
        current_minutes = current_h * 60 + current_m

        # Extended hours: 08:00 (480 min) to 22:00 (1320 min)
        business_end = 22 * 60  # 22:00

        # Calculate how much time is left
        time_left = business_end - current_minutes

        # If less than 10 minutes left, no meetings possible
        if time_left < 10:
            return []

        # Ensure earliest meeting is at least 2 hours (120 minutes) from current time
        if time_left < 120:
            # Not enough time for 2-hour gap - no meetings possible
            return []
        else:
            # Normal hours - ensure at least 2 hours gap, up to 3 hours
            min_gap = 120  # 2 hours minimum
            max_gap = min(180, time_left - 30)  # Up to 3 hours, but leave 30 min buffer

        gap = random.randint(min_gap, max(min_gap, max_gap))
        earliest_start = current_minutes + gap

        # Generate random number of meetings (0-5)
        num_meetings = random.randint(0, 5)

        meetings = []
        available_start = earliest_start

        # Possible durations (in minutes) - varied
        if time_left < 60:
            # Short meetings only when time is limited
            durations = [15, 20, 25, 30]
        else:
            durations = [30, 40, 45, 50, 60, 75, 80, 90, 100, 105, 110, 120]

        for _ in range(num_meetings):
            # Check if enough time left for even a short meeting
            remaining = business_end - available_start
            if remaining < 15:
                break

            # Random duration (or shorter if needed)
            duration = random.choice(durations)
            # Ensure duration fits in remaining time
            duration = min(duration, remaining - 5)

            # Ensure meeting ends before extended hours
            latest_start = business_end - duration
            if available_start > latest_start:
                break

            # Random start time (add 0-25 min variation to avoid round hours)
            variation = random.choice([0, 5, 10, 15, 20, 25])
            start_minutes = available_start + variation

            # Ensure we don't go past latest_start
            if start_minutes > latest_start:
                start_minutes = latest_start

            end_minutes = start_minutes + duration

            # Ensure end is before extended hours
            if end_minutes > business_end:
                end_minutes = business_end
                duration = end_minutes - start_minutes
                if duration < 30:  # Skip if too short
                    break

            # Convert to HH:MM format
            start_h, start_m = divmod(start_minutes, 60)
            end_h, end_m = divmod(end_minutes, 60)

            meetings.append({
                "start_time": f"{start_h:02d}:{start_m:02d}",
                "end_time": f"{end_h:02d}:{end_m:02d}"
            })

            # Next meeting starts after current one + gap (30 min to 3 hours)
            gap = random.randint(30, 180)
            available_start = end_minutes + gap

        return meetings
