...
class HVACAgent(STARAgent):
    ...
    def __init__(self, **kwargs):
        ...
        super().__init__(agent_id="hvac-agent", llm_provider="llamastack", model="openai/gpt-4.1", prompt_path=prompt_path, **kwargs)
...
class LearningAgent(STARAgent):
    ...
    def __init__(self, **kwargs):
        # Set prompt path
        prompt_path = os.path.join(os.path.dirname(__file__), "prompts", "LearningAgent.xml")

        super().__init__(agent_id="learning-agent", llm_provider="llamastack", model="openai/gpt-4.1", prompt_path=prompt_path, **kwargs)
...
def generate_feedback_summary(feedback, env_status, agent_plan):
    ...
    if feedback["action_results"]:
        ...
        for i, action in enumerate(feedback["action_results"]):
            ...
            if action["time_available_minutes"] is not None:
                ...
                summary_parts.append(f"    Time Available: {time_available} min")
                    ...
                    try:
                        ...
                        if time_diff_minutes > 0:
                            ...
                            if time_diff_minutes > 5:
                                ...
                                summary_parts.append(f"    ✓ Reached {time_diff_minutes:.1f} min before meeting (good timing)")
                        else:
                            # Target reached after meeting started
                            summary_parts.append(f"    ❌ Target reached {abs(time_diff_minutes):.1f} min AFTER meeting started!")
                    except ValueError:
        ...
        summary_parts.append(f"  Sum of Action Costs: {total_cost_check:.3f} kWh")
        ...
        if cost_diff > 0.001:
            summary_parts.append(f"  ⚠️  MISMATCH: Difference of {cost_diff:.3f} kWh")
        else:
    ...
    if feedback["plan_success"] == "success":
        ...
        summary_parts.append(f"  • {action_verb} from {indoor_temp}°F to {target_temp}°F")
            ...
            if action["schedule_success"] == "success":
                ...
                if "meeting_start_time" in action:
                    ...
                    try:
                        ...
                        if time_diff_minutes > 0:
                            if time_diff_minutes > 5:
                                meeting_timing_info = f" (meeting at {meeting_start}, wasted: {time_diff_minutes:.1f} min)"
                            else:
                                meeting_timing_info = f" (meeting at {meeting_start}, good timing: {time_diff_minutes:.1f} min early)"
                        else:
                            meeting_timing_info = (
                                f" (meeting at {meeting_start}, LATE: {abs(time_diff_minutes):.1f} min after meeting started!)"
                            )
                    except ValueError:
                        pass
                ...
                error = action["error"]
                if COLORAMA_AVAILABLE:
                    summary_parts.append(f"    {i + 1}. {time_range}: {Fore.RED}✗ Failed{Style.RESET_ALL}: {error}")
                else:
                    summary_parts.append(f"    {i + 1}. {time_range}: \033[91m✗ Failed\033[0m: {error}")
    else:
        if COLORAMA_AVAILABLE:
        ...
        for i, action in enumerate(feedback["action_results"]):
            ...
            if action["schedule_success"] == "success":
                ...
                cost = action["cost_kwh"]
                if COLORAMA_AVAILABLE:
                    summary_parts.append(f"    {Fore.RED}✗{Style.RESET_ALL} {i + 1}. {time_range}: Failed (cost: {cost:.3f} kWh)")
                else:
                    summary_parts.append(f"    \033[91m✗\033[0m {i + 1}. {time_range}: Failed (cost: {cost:.3f} kWh)")
...
def analyze_feedback_with_learning_agent(feedback, env_status, agent_plan):
    ...
    feedback_summary = generate_feedback_summary(feedback, env_status, agent_plan)
...
"""
    ...
    return {"success": True, "insights": learning_insights, "raw_response": learning_insights}
...
def run_hvac_flow():
    ...
    env_status = {
        ...
        "outdoor_temp": 92.4,
        "meeting_plan": [
            {"start_time": "15:37", "end_time": "17:27"},
            {"start_time": "18:22", "end_time": "18:52"},
...
"""
    ...
    learning_analysis = analyze_feedback_with_learning_agent(feedback, env_status, agent_plan)
