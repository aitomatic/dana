"""
DEPRECATED: Dana Core Runtime Registries

This location is deprecated. All registries have been moved to dana.registries for cleaner imports.

Please update your imports:
  OLD: from dana.core.runtime.registries import AgentTypeRegistry
  NEW: from dana.registries import AgentTypeRegistry

This module provides backward compatibility imports and will be removed in a future version.

Copyright © 2025 Aitomatic, Inc.
MIT License
"""

# Backward compatibility imports from new location
import warnings

# Issue deprecation warning
warnings.warn("dana.core.runtime.registries is deprecated. Use dana.registries instead.", DeprecationWarning, stacklevel=2)

# Import all registries from new location for backward compatibility
from dana.registries import *  # noqa: F403, F401

# Re-export everything to maintain compatibility
from dana.registries import __all__  # noqa: F401
