...
class ConversationMemory:
    ...
    def __init__(self, filepath: str | None = None, max_turns: int = 20):
        """
        Initialize conversation memory.

        Args:
            filepath: Path to JSON file for persistence (optional)
            max_turns: Maximum number of turns to keep in active memory
        """
        if filepath:
            self.filepath = Path(filepath)
        else:
            # Default to user's chat directory
            default_dir = Path("~/.dana/chats").expanduser()
            default_dir.mkdir(parents=True, exist_ok=True)
            self.filepath = default_dir / f"conversation_{uuid.uuid4()}.json"
    ...
    def get_recent_turns(self, n_turns: int = 5) -> list[dict]:
        """Alias for get_recent_context for compatibility."""
        return self.get_recent_context(n_turns)

    def search(self, query: str, max_results: int = 5) -> list[dict]:
        """Alias for search_history for compatibility."""
        return self.search_history(query, max_results)
