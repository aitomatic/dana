...
def create_branching_fsm(states: list[str], initial_state: str, transitions: dict[tuple[str, str], str]) -> dict[str, Any]:
    """
...
def is_terminal_state(fsm: Any, state: str) -> bool:
    ...
    for (from_state, _), _ in fsm.transitions.items():
        if from_state == state:
            return False
    return True
...
def get_available_events(fsm: Any, state: str) -> list[str]:
    ...
    events = []
    for (from_state, event), _ in fsm.transitions.items():
        if from_state == state:
            events.append(event)
    return events
