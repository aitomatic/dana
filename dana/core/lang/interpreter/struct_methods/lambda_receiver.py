    ...
    def create_method_function(self):
        ...
        def method_function(receiver_instance: Any, *args, **kwargs):
            ...
            if not self._validate_receiver_instance(receiver_instance):
                ...
                raise SandboxError(
                    f"Invalid receiver type for lambda method. Expected one of {receiver_types}, "
                    f"got {type(receiver_instance)}"
                )
            ...
            method_context.set_variable(self.receiver.name, receiver_instance)
            
            # Bind regular parameters
            for i, param in enumerate(self.parameters):
                if i < len(args):
                    method_context.set_variable(param.name, args[i])
                elif param.name in kwargs:
                    method_context.set_variable(param.name, kwargs[param.name])
