...
from typing import Any, List, Optional, Union as TypingUnion
...
class UnionTypeHandler:
    ...
    @staticmethod
    def is_union_type(type_hint: Optional[TypeHint]) -> bool:
...
class LambdaUnionReceiver:
    ...
    def create_union_method_function(self):
        ...
        def union_method_function(receiver_instance: Any, *args, **kwargs):
            ...
            if runtime_type not in self.union_types:
                raise SandboxError(
                    f"Runtime type '{runtime_type}' does not match union types {self.union_types}"
                )
    ...
    def _execute_lambda_with_receiver(self, receiver_instance: Any, *args, **kwargs) -> Any:
        ...
        # Bind receiver
        lambda_context.set_variable(self.receiver.name, receiver_instance)
...
class UnionLambdaDispatcher:
    ...
    def can_handle_union_method_call(obj: Any, method_name: str) -> bool:
        ...
        return (method is not None and 
                hasattr(method, '_dana_lambda_union_types') and
                struct_type.name in method._dana_lambda_union_types)
    ...
    def dispatch_union_method_call(obj: Any, method_name: str, *args, **kwargs) -> Any:
        ...
        if struct_type.name not in union_types:
            raise SandboxError(
                f"Type '{struct_type.name}' is not in union types {union_types} for method '{method_name}'"
            )
...
def enhance_lambda_receiver_with_unions():
    ...
    try:
        ...
        original_get_receiver_types = LambdaReceiver.get_receiver_types
