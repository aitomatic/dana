...
class LambdaTypeInferencer:
    ...
    def infer_lambda_type(self, lambda_expr: LambdaExpression, context_type: Optional[DanaType] = None) -> DanaType:
        ...
        try:
            ...
            for param, param_type in zip(lambda_expr.parameters, param_types):
    ...
    def _infer_receiver_type(self, receiver: Optional[Parameter], context_type: Optional[DanaType]) -> Optional[DanaType]:
        ...
        if context_type and context_type.name.startswith("function"):
            # Parse function type to extract receiver type
            # This is simplified - a full implementation would parse function signatures
            return DanaType("any")
        
        # Default to any if no information available
        return DanaType("any")

    def _infer_parameter_types(self, parameters: list[Parameter], context_type: Optional[DanaType]) -> list[DanaType]:
    ...
    def _create_function_type(self, receiver_type: Optional[DanaType], param_types: list[DanaType], return_type: DanaType) -> DanaType:
