...
from dana.api.services.intent_detection.intent_handlers.handler_prompts.knowledge_ops_prompts import GENERATE_QUESTION_PROMPT, ACCESS_COVERAGE_PROMPT, KNOWLEDGE_EXTRACTION_PROMPT, KNOWLEDGE_GENERATION_PROMPT
import logging
import asyncio
import re
...
class GenerateKnowledgeTool(BaseTool):
    ...
    async def _generate_for_all_leaves(self, user_message: str, counts: str, context: str) -> ToolResult:
        ...
        for i, path in enumerate(all_leaf_paths):
            ...
            if self.notifier:
                await self.notifier(
                    "generate_knowledge", f"✅ Completed '{leaf_topic}' - {i+1}/{len(all_leaf_paths)} done", "in_progress", (i+1)/len(all_leaf_paths)
                )
        ...
        return ToolResult(name="generate_knowledge", result=self._build_structured_response(user_message, f"✅ Knowledge generation complete. Knowledge overview: \n{status_text}"), require_user=False)
    ...
    async def _generate_questions_for_topic_paths(self, paths: list[str]) -> str:
        ...
        while (current_confidence < 85) and (count < 15):
            count += 1
            new_questions = await asyncio.to_thread(reason, GENERATE_QUESTION_PROMPT.format(path=path, tasks=tasks, role=self.role, domain=self.domain, confidence=current_confidence, suggestion=suggestion, questions=questions))
            questions = questions + f"\n{new_questions}"
            confidence_result = await asyncio.to_thread(reason, prompt=ACCESS_COVERAGE_PROMPT.format(questions=questions, role=self.role, 
            domain=self.domain, tasks=tasks, confidence=current_confidence), target_type=dict)
    ...
    async def _generate_knowledge_for_topic_paths(self, paths: list[str], questions: str) -> list[RawFormatKnowledge]:
        ...
        async def generate_knowledge(question: str, chunks: list[NodeWithScore], path_str: str) -> RawFormatKnowledge:
            if chunks:
                res = await asyncio.to_thread(reason, KNOWLEDGE_EXTRACTION_PROMPT.format(path=path_str, question=question, chunks=_format_chunks(chunks)))
                from_doc = True
            else:
                res = await asyncio.to_thread(reason, KNOWLEDGE_GENERATION_PROMPT.format(path=path_str, question=question, role=self.role, domain=self.domain))
                from_doc = False
        ...
        async_tasks = []
        path_str = " → ".join(paths)
        for question, chunks in zip(question_list, relevant_chunks):
            if question:
    ...
    async def _transform_knowledge_units(self, knowledge_units: list[RawFormatKnowledge]) -> list[Knowledge]:
        ...
        for unit in knowledge_units:
            ...
            if len(sections) > 1:
                sections = sections[1:]
            for section, section_content in zip(matches, sections):
                if unit.from_doc is True and "[Chunk" in section_content:
                    ...
                    for part in parts:
                        if "[Chunk" in part:
                            ...
                            if ref_chunk_idxs:
                                knowledge_unit = KnowledgeUnit(content=part.strip(), references=[Reference(source=chunks[idx].node.metadata["source"], page_number=chunks[idx].node.metadata.get("page_label", 0)) for idx in ref_chunk_idxs])
...
if __name__ == "__main__":
    import json
    with open("agents/domain_knowledge/domain_knowledge.json", "r") as f:
