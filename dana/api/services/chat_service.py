...
class ChatService:
    ...
    def _get_prebuilt_agent_folder(self, agent_key: str) -> Path | None:
        ...
        """
        prebuilt_folder = (
            Path(__file__).parent.parent / "server" / "assets" / agent_key
        )
    ...
    def _get_prebuilt_agent_info(self, agent_key: str) -> dict | None:
        ...
        try:
            ...
            return next(
                (agent for agent in prebuilt_agents if agent["key"] == agent_key), 
                None
            )
        except Exception as e:
    ...
    async def _generate_prebuilt_agent_response(
        self, chat_request: ChatRequest, agent_name: str, agent_description: str, 
        folder_path: str, websocket_id: str | None = None
    ) -> str:
