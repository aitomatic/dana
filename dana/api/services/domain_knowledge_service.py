...
class DomainKnowledgeService(Loggable):
        ...
        try:
            ...
            if not tree:
                return DomainKnowledgeUpdateResponse(
                    success=False,
                    error="No domain knowledge tree found for agent"
                )
            ...
            if self._find_node(tree.root, topic):
                return DomainKnowledgeUpdateResponse(
                    success=False,
                    error=f"Topic '{topic}' already exists in tree"
                )
    ...
    async def _save_version_to_history(self, agent_id: int, tree: DomainKnowledgeTree) -> None:
