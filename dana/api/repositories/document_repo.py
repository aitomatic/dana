...
class AbstractDocumentRepo(ABC):
    @classmethod
    @abstractmethod
    async def get_extraction(cls, document_id: int, deep_extract: bool = False, **kwargs) -> ExtractionOutput | None:
        pass
...
class SQLDocumentRepo(AbstractDocumentRepo):
    ...
    async def get_extraction(cls, document_id: int, deep_extract: bool = False, **kwargs) -> ExtractionOutput | None:
        ...
        for extracted_document in extracted_documents:
            if extracted_document.doc_metadata.get("deep_extracted") == deep_extract:
