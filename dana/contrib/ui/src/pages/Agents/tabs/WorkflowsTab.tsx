...
const WorkflowsTab: React.FC = () => {
      ...
      setExecutingWorkflows(prevExecutions => {
        ...
        for (const [executionId, execution] of updated.entries()) {
          if (execution.status === ExecutionStatus.RUNNING) {
            ...
            apiService.getWorkflowExecutionStatus(executionId)
              .then(status => {
                ...
                console.log(`📊 Step ${execution.currentStep + 1} completed with result:`, execution.stepResults[execution.currentStep]?.result);
  ...
  const generateAgentSpecificTestData = (agentId: number, workflowName: string, customQuery?: string) => {
  ...
  const startWorkflowExecution = async (workflow: WorkflowDefinition) => {
    try {
      ...
      const testData = generateAgentSpecificTestData(parseInt(agentId || '0'), workflow.name, customTestQuery);
      ...
      console.log(`🚀 Starting workflow '${workflow.name}' with agent-specific input:`, request.input_data);
    ...
    return (
      <div className="p-8">
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-gray-500">This agent doesn't have any workflow definitions yet.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="px-6 pb-6">
      ...
      <div className="mb-6">
        ...
        <div className="mb-6 p-6 bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 rounded-xl border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">Configure test parameters for workflow validation</p>
              </div>
            </div>
              ...
              <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
                ...
                <div className="mt-2 flex items-center justify-between">
                  <p className="text-xs text-slate-500">
                    💡 <span className="font-medium">Pro tip:</span> Use specific queries to test edge cases and workflow robustness
                  </p>
                  <div className="flex items-center space-x-2">
                    <button
              ...
              <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
                ...
                <div className="space-y-2">
                  <button
                    ...
                    className="w-full px-3 py-2 bg-emerald-50 text-emerald-700 text-xs font-medium rounded-md hover:bg-emerald-100 transition-colors text-left"
                  >
                    🧪 Edge Case Test
                  </button>
                  <button
                    onClick={() => setCustomTestQuery("Performance test with complex query")}
                    className="w-full px-3 py-2 bg-blue-50 text-blue-700 text-xs font-medium rounded-md hover:bg-blue-100 transition-colors text-left"
                  >
                    ⚡ Performance Test
                  </button>
                  <button
                    onClick={() => setCustomTestQuery("Integration test with external data")}
                    className="w-full px-3 py-2 bg-purple-50 text-purple-700 text-xs font-medium rounded-md hover:bg-purple-100 transition-colors text-left"
                  >
                    🔗 Integration Test
                  </button>
                </div>
          ...
          <div className="flex items-center">
            ...
            <div className="ml-3">
              ...
              <p className="text-2xl font-bold text-gray-900">
                {Array.from(executingWorkflows.values()).filter(w => w.status === ExecutionStatus.COMPLETED).length}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              ...
              <p className="text-2xl font-bold text-gray-900">
                {Array.from(executingWorkflows.values()).filter(w => w.status === ExecutionStatus.RUNNING).length}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              ...
              <p className="text-2xl font-bold text-gray-900">
                {Array.from(executingWorkflows.values()).filter(w => w.status === ExecutionStatus.FAILED).length}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          return (
            <div 
              key={workflow.name}
              className={`p-6 bg-gradient-to-br from-white to-gray-50 rounded-xl border border-gray-200 shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 ${
            ...
            >
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <div className="bg-white px-3 py-2 rounded-lg border border-gray-100 inline-block">
                    <span className="text-sm text-gray-600">{workflow.description}</span>
                  </div>
                </div>
                
                {/* Execution Status Badge */}
                <div className={`inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold border ${getExecutionStatusColor(execution?.status || ExecutionStatus.IDLE)}`}>
                  {getExecutionStatusIcon(execution?.status || ExecutionStatus.IDLE)}
                  <span className="ml-2 capitalize">{execution?.status || ExecutionStatus.IDLE}</span>
                </div>
              </div>
                 ...
                 <div className="mb-4 p-4 bg-gray-50 rounded-lg">
                   <div className="flex items-center justify-between mb-2">
                     ...
                     <span className="text-sm text-gray-500">
                       Step {Math.min(execution.currentStep + 1, execution.totalSteps)} of {execution.totalSteps}
                     </span>
                   </div>
                   ...
                   <div className="flex items-center mt-2 text-sm text-gray-500">
                     ...
                     {execution.executionTime > 0 ? `${Math.round(execution.executionTime)}s` : 'Starting...'}
                   </div>
                   ...
                   <div className="mt-3 p-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
                       ...
                       <div className="grid grid-cols-2 gap-2 text-xs">
                         <div>
                           ...
                           <span className={`ml-1 px-1 py-0.5 text-xs rounded ${
                             execution.status === ExecutionStatus.COMPLETED ? 'bg-green-100 text-green-700' :
                             execution.status === ExecutionStatus.RUNNING ? 'bg-blue-100 text-blue-700' :
                             execution.status === ExecutionStatus.FAILED ? 'bg-red-100 text-red-700' :
                     ...
                     <div className="grid grid-cols-2 gap-2 text-xs">
                       <div>
                         ...
                         <div className="text-blue-700 truncate mt-1 p-1 bg-blue-100 rounded border border-blue-200">
                       ...
                       <div>
                         ...
                         <div className="text-green-700 truncate mt-1 p-1 bg-green-100 rounded border border-green-200">
                           {execution.stepResults[execution.stepResults.length - 1]?.result ? 
                             (typeof execution.stepResults[execution.stepResults.length - 1].result === 'object' ? 
                               JSON.stringify(execution.stepResults[execution.stepResults.length - 1].result).substring(0, 60) + '...' : 
                               String(execution.stepResults[execution.stepResults.length - 1].result).substring(0, 60) + '...'
                             ) : 
                             'No output data'
                           }
                         </div>
                     ...
                     {/* Execution Metadata */}
                     {execution.execution_metadata && Object.keys(execution.execution_metadata).length > 0 && (
                       <div className="mt-2 p-2 bg-white rounded border border-blue-100">
                         <div className="text-xs text-blue-600 font-medium mb-1">⚡ Runtime Metrics</div>
                         <div className="grid grid-cols-2 gap-1 text-xs">
                           <div>
                             <span className="text-gray-600">Memory:</span>
                             <span className="ml-1 text-gray-700">
                               {execution.execution_metadata.memory_usage ? 
                                 `${execution.execution_metadata.memory_usage}MB` : 'N/A'}
                             </span>
                           </div>
                           <div>
                     ...
                     <button
                       ...
                       className="mt-2 w-full text-xs bg-blue-100 hover:bg-blue-200 text-blue-800 px-2 py-1 rounded transition-colors"
                     >
                    ...
                    return (
                      <div key={stepIndex} className="relative">
                        ...
                        <div className="flex items-center mb-3">
                          {/* Step Number Circle */}
                          <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center mr-4 z-10 relative ${
                            stepStatus === 'completed' ? 'bg-gradient-to-br from-green-500 to-green-600' :
                            stepStatus === 'running' ? 'bg-gradient-to-br from-blue-500 to-blue-600 animate-pulse' :
                            stepStatus === 'failed' ? 'bg-gradient-to-br from-red-500 to-red-600' :
              ...
              <div className="flex space-x-3">
                {!execution || execution.status === ExecutionStatus.IDLE || execution.status === ExecutionStatus.COMPLETED || execution.status === ExecutionStatus.FAILED ? (
                  <button
      ...
      {selectedWorkflow && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Workflow Details: {selectedWorkflow.name}</h3>
                ...
                if (execution) {
                  return (
                    <div className="mb-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                      <h4 className="text-sm font-medium text-blue-800 mb-2">Execution Summary</h4>
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          <span className="text-blue-600 font-medium">Status:</span>
                          <span className={`ml-2 px-2 py-1 text-xs rounded-full ${
                            execution.status === ExecutionStatus.COMPLETED ? 'bg-green-100 text-green-800' :
                            execution.status === ExecutionStatus.RUNNING ? 'bg-blue-100 text-blue-800' :
                            execution.status === ExecutionStatus.FAILED ? 'bg-red-100 text-red-800' :
                        ...
                        <div>
                          <span className="text-blue-600 font-medium">Execution Time:</span>
                          <span className="ml-2 text-blue-700">
                            {execution.executionTime ? `${Math.round(execution.executionTime)}s` : 'Starting...'}
                          </span>
                        </div>
                        <div>
                          <span className="text-blue-600 font-medium">Started:</span>
                          <span className="ml-2 text-blue-700">
                            {execution.startTime ? execution.startTime.toLocaleTimeString() : 'Unknown'}
                          </span>
                        </div>
                      </div>
                    </div>
              ...
              <div className="space-y-4">
                      ...
                      {(() => {
                        ...
                        return (
                          <>
                            ...
                            {testData.agent_type && (
                              <div className="mb-2">
                                <span className="font-medium">Agent Type:</span> {testData.agent_type}
                              </div>
                            )}
                            ...
                            {'customer_tier' in testData && testData.customer_tier && (
                              <div className="mb-2">
                                <span className="font-medium">Customer Tier:</span> {testData.customer_tier}
                              </div>
                            )}
                            ...
                            <div className="mb-2">
                              <span className="font-medium">Timestamp:</span> {testData.timestamp ? new Date(testData.timestamp).toLocaleString() : new Date().toLocaleString()}
                            </div>
                          </>
                        );
                      })()}
                  ...
                  <div className="space-y-2">
                    {selectedWorkflow.steps.map((step, index) => (
                      <div key={index} className="flex items-center space-x-3">
                        ...
                        <span className="text-sm text-gray-800 font-mono">{step}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {(() => {
                  ...
                  if (execution && execution.stepResults.length > 0) {
                    return (
                      <div>
                        <h4 className="text-sm font-medium text-gray-700 mb-2">Execution Data</h4>
                        
                        {/* Real-time Execution Status */}
                        <div className="mb-4 p-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-sm font-medium text-blue-800">🔄 Real-time Execution</span>
                              ...
                              <div>
                                ...
                                <span className="ml-1 text-blue-700">
                                  {execution.execution_metadata.execution_time ? 
                                    `${execution.execution_metadata.execution_time.toFixed(2)}s` : 'N/A'}
                                </span>
                              </div>
                              <div>
                              ...
                              <div>
                                ...
                                <span className="ml-1 text-blue-700">
                                  {execution.execution_metadata.errors ? 
                                    execution.execution_metadata.errors.length : 0}
                                </span>
                              </div>
                            </div>
                          )}
                              ...
                              <div className="flex items-center justify-between mb-3">
                                ...
                                <div className="flex items-center space-x-2">
                                  <span className={`px-2 py-1 text-xs rounded-full ${
                                    ...
                                    stepResult.status === 'running' ? 'bg-blue-100 text-blue-800' :
                               ...
                               <div className="space-y-3">
                                 ...
                                 <div className="flex items-center justify-between text-xs">
                                   ...
                                   <span className={`px-2 py-1 rounded ${
                                     stepResult.status === 'completed' ? 'bg-green-100 text-green-700' :
                                     stepResult.status === 'running' ? 'bg-blue-100 text-blue-700' :
                                ...
                                {stepResult.error && (
                                  <div>
                                    ...
                                    <div className="mt-1 p-2 bg-red-50 rounded text-xs font-mono text-red-700 max-h-32 overflow-y-auto border border-red-200">
                                      {stepResult.error}
                                    </div>
                                  </div>
                                )}
                                ...
                                {stepResult.startTime && stepResult.endTime && (
                                  <div className="flex items-center justify-between text-xs text-gray-500">
                                    <span>Started: {stepResult.startTime.toLocaleTimeString()}</span>
                        ...
                        {execution.result && (
                          <div className="mt-4 p-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200">
                            <h5 className="text-sm font-medium text-green-800 mb-2">🎯 Final Result</h5>
                            <div className="p-2 bg-green-100 rounded text-xs font-mono text-green-800 max-h-32 overflow-y-auto">
                        ...
                        {execution.error && (
                          <div className="mt-4 p-3 bg-gradient-to-r from-red-50 to-pink-50 rounded-lg border border-red-200">
                            <h5 className="text-sm font-medium text-red-800 mb-2">❌ Execution Error</h5>
                            <div className="p-2 bg-red-100 rounded text-xs font-mono text-red-800 max-h-32 overflow-y-auto">
                              {execution.error}
                            </div>
                          </div>
                        )}
                  ...
                  return (
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p className="text-xs text-gray-400 mt-1">Execute the workflow to see real-time data flow!</p>
                    </div>
                  );
                })()}
              </div>
