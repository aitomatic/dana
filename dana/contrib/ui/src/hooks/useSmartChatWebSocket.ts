...
export const useSmartChatWebSocket = ({
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      console.log(`${reconnectAttemptsRef.current > 0 ? 'Reconnecting' : 'Connecting'} WebSocket for agent ${agentId}...`);
      ...
      ws.onclose = (event) => {
        console.log(`🔌 WebSocket disconnected for agent ${agentId}`, { code: event.code, reason: event.reason });
        ...
        if (shouldReconnect) {
          ...
          reconnectTimeoutRef.current = setTimeout(() => {
            if (enabled && agentId) { // Double-check conditions before reconnecting
              connect();
            }
          }, reconnectDelay);
        } else {
          console.log(`❌ Not reconnecting: enabled=${enabled}, code=${event.code}, attempts=${reconnectAttemptsRef.current}/${maxReconnectAttempts}`);
        }
      };
