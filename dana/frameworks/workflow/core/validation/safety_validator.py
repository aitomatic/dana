...
class SafetyValidator:
        ...
        try:
            ...
            if len(sig.parameters) == 0:
                return SafetyResult(
                    is_safe=True,
                    level=SafetyLevel.SAFE,
                    reason="Function has no parameters",
                    details={"name": name},
                    recommendations=[]
                )
    ...
    def _rule_basic_structure(self, workflow: Any, context: Any = None) -> SafetyResult:
        ...
        if isinstance(workflow, list) and len(workflow) == 0:
            return SafetyResult(
                is_safe=False,
                level=SafetyLevel.ERROR,
                reason="Empty workflow list",
                details={},
                recommendations=["Add at least one step"]
            )
        
        return SafetyResult(
            is_safe=True,
            level=SafetyLevel.SAFE,
            reason="Basic structure valid",
            details={},
            recommendations=[]
        )
    ...
    def _rule_no_infinite_recursion(self, workflow: Any, context: Any = None) -> SafetyResult:
        ...
        return SafetyResult(
            is_safe=True,
            level=SafetyLevel.SAFE,
            reason="No infinite recursion detected",
            details={},
            recommendations=[]
        )
    ...
    def _rule_reasonable_complexity(self, workflow: Any, context: Any = None) -> SafetyResult:
        ...
        return SafetyResult(
            is_safe=True,
            level=SafetyLevel.SAFE,
            reason="Complexity within acceptable limits",
            details={},
            recommendations=[]
        )
