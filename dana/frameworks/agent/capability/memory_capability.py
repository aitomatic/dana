...
class MemoryCapability(BaseCapability):
    ...
    def __init__(self, max_size: int | None = None, decay_rate: float = 0.1, importance_threshold: float = 0.5):
        ...
        """
        super().__init__(name="memory", description="Memory and state management capability")
    ...
    async def apply(self, context: dict[str, Any], **kwargs) -> dict[str, Any]:
        ...
        if operation == "store":
            return await self._store_memory(kwargs.get("content"), kwargs.get("metadata", {}), kwargs.get("importance", 1.0), context)
        if operation == "retrieve":
            return await self._retrieve_memories(kwargs.get("query", {}), kwargs.get("limit"))
        if operation == "update_working":
            return await self._update_working_memory(kwargs.get("updates", {}))
        if operation == "clear":
    ...
    async def _store_memory(self, content: Any, metadata: dict, importance: float, context: dict) -> dict[str, Any]:
        ...
        try:
            ...
            entry = MemoryEntry(content=content, metadata=metadata, importance=importance, context=context)
    ...
    async def _retrieve_memories(self, query: dict, limit: int | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            # Filter memories
            matches = [mem for mem in self._memories if self._matches_query(mem, query)]
            ...
            return {
                "success": True,
                "memories": [
                    {"content": mem.content, "timestamp": mem.timestamp, "importance": mem.importance, "metadata": mem.metadata}
                    for mem in matches
                ],
            }
    ...
    def _cleanup_memories(self):
        ...
        self._memories = [mem for mem in self._memories if mem.importance >= self.importance_threshold]
